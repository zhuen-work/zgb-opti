...
WINDOWS = [
    ...
    ("w3", date(2026,3,14), date(2026,4,11), date(2026,4,11), date(2026,4,21)),
]

OUTPUT_DIR    = Path("output/scalp_wfo_apr21")
SET_OUT       = Path("configs/sets/scalp_v1_wfo_apr21.set")
...
def _base_params(donch, tp, sl, ma_period, htp):
    ...
    return [
        ...
        _fix("_MA_Period", ma_period),
        _fix("_MA_Method", 0),  # SMA
...
def _opt_params_sweep():
    ...
    return [
        ...
        _fix("_MaxPositions", 2),
        _fix("_EntryTF", 5),
        _sweep("_DonchianBars", 20, 5, 15, 30),       # 4 values
        _sweep("_TakeProfit",   150, 50, 100, 200),   # 3 values
        _sweep("_StopLoss",     75, 25, 50, 100),     # 3 values
        _sweep("_HalfTP_Ratio", 0.5, 0.3, 0.0, 0.8),  # 3 values: 0.0,0.3,0.6
        _fix("_PendingExpireBars", 1),
        _sweep("_MA_Period", 50, 20, 0, 100),         # 6 values: 0,20,40,60,80,100
        _fix("_MA_Method", 0),  # SMA only
        _fix("_DailyTargetPct", TARGET_PCT),
        _fix("_DailyLossPct", LOSS_PCT),
        _fix("_TradeStartHour", 14),
        _fix("_TradeEndHour", 22),
...
def run_window_optimization(win_name, is_start, is_end):
    ...
    ma_c   = [c for c in df.columns if "_MA_Period" in c][0]

    for c in [donch_c, tp_c, sl_c, htp_c, ma_c]:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    top20 = df.sort_values("rf", ascending=False).head(20)
    print(f"  Window {win_name}: top 5 by RF:")
    print(f"    {'NP':>10}  {'PF':>6}  {'DD':>6}  {'Tr':>4}  {'RF':>6}  Donch  TP  SL  MA  HTP")
    for _, r in top20.head(5).iterrows():
        print(f"    {r['net_profit']:>+10,.0f}  {r['profit_factor']:>6.2f}  {r['drawdown_pct']:>5.1f}%  "
              ...
              f"{int(r[sl_c]):>3}  {int(r[ma_c]):>3}  {r[htp_c]:>4.1f}")

    # Return list of (donch, tp, sl, adx, htp, rf) tuples
    results = []
...
def main():
    ...
    def param_key(p):
        return (p["donch"], p["tp"], p["sl"], p["ma"], p["htp"])
    ...
    for i, (k, info) in enumerate(sorted(robust.items(), key=lambda x: -x[1]["total_rf"])[:10], 1):
        donch, tp, sl, ma, htp = k
        params = {"donch": donch, "tp": tp, "sl": sl, "ma": ma, "htp": htp}
        print(f"\n  #{i} Donch={donch} TP={tp} SL={sl} MA={ma} HTP={htp}  (in {info['count']} windows: {info['windows']})")
    ...
    for rank, s in enumerate(robust_scores[:5], 1):
        ...
        print(f"  {rank:<4}  {s['oos_total_np']:>+12,.0f}  {flag:>8}  Donch={p['donch']} TP={p['tp']} SL={p['sl']} MA={p['ma']} HTP={p['htp']}")
    ...
    if robust_scores:
        ...
        set_content = "\n".join([
            ...
            f"_MA_Period=14||14||1||14||14||N",
            f"_MA_Period={p['ma']}||{p['ma']}||1||{p['ma']}||{p['ma']}||N",
            f"_MA_Method=0||0||1||0||0||N",
