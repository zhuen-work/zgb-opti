...
OUTPUT_DIR    = Path("output/scalp_wfo_apr18")
SET_OUT       = Path("configs/sets/scalp_v1_wfo_apr18.set")
...
def _base_params(donch, tp, sl, htp):
...
def _opt_params_sweep():
    ...
    return [
        ...
        _sweep("_HalfTP_Ratio", 0.3, 0.3, 0.0, 0.9),  # 4 values: 0.0,0.3,0.6,0.9
        _fix("_PendingExpireBars", 1),
        _fix("_DailyTargetPct", TARGET_PCT),
        _fix("_DailyLossPct", LOSS_PCT),
        _fix("_TradeStartHour", 14),
...
def main():
    ...
    def param_key(p):
        return (p["donch"], p["tp"], p["sl"], p["htp"])
    ...
    for i, (k, info) in enumerate(sorted(robust.items(), key=lambda x: -x[1]["total_rf"])[:10], 1):
        ...
        print(f"\n  #{i} Donch={donch} TP={tp} SL={sl} HTP={htp}  (in {info['count']} windows: {info['windows']})")
    ...
    for rank, s in enumerate(robust_scores[:5], 1):
        ...
        print(f"  {rank:<4}  {s['oos_total_np']:>+12,.0f}  {flag:>8}  Donch={p['donch']} TP={p['tp']} SL={p['sl']} HTP={p['htp']}")
