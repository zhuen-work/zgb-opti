"""DT818_exp reopt: S1=H1, S2=H4 (BB breakout). EMA fixed at 2.

Joint genetic sweep, ET-validates top 10 distinct passes at 4% risk.
Retries up to 3x if no pass meets gate (trades <= 150, NP >= $10K, DD <= 20%).

IS period: Jan 24 -> Mar 21, 2026.  Chart period: H1.
"""
...
OUTPUT_DIR    = Path(f"output/dt818_exp_h1xh4_{IS_TAG}")
...
def _joint_set_lines():
    return [
        ...
        _sweep("_take_profit",  12000, 1000, 4000,  28000),
        _sweep("_stop_loss",    8000,  500,  3000,  15000),
        _fix("_EMA_Period1", 2),
        _sweep("_BB_Period1", 20, 5, 10, 50),
        _sweep("_BB_Dev1",  "2.0", "0.5", "1.5", "3.0"),
        _fix("_RiskMode1", 0),
        _sweep("_Trade2",   1, 1, 0, 1),
        _fix("_time_frame2", TF2_ENUM),
        _sweep("_take_profit2", 16000, 1000, 6000,  35000),
        _sweep("_stop_loss2",   10000, 500,  4000,  18000),
        _fix("_EMA_Period2", 2),
        _sweep("_BB_Period2", 20, 5, 10, 50),
        _sweep("_BB_Dev2",  "2.0", "0.5", "1.5", "3.0"),
        _fix("_RiskMode2", 0),
    ]
...
def _build_val_ini(p, val_id):
    val_inputs = "\n".join([
        ...
        f"_take_profit={p['take_profit']}",
        f"_stop_loss={p['stop_loss']}",
        "_EMA_Period1=2",
        f"_BB_Period1={p['BB_Period1']}",
        f"_BB_Dev1={p['BB_Dev1']}",
        "_RiskMode1=0",
        "_Trade2=1",
        f"_time_frame2={TF2_ENUM}",
        f"_take_profit2={p['take_profit2']}",
        f"_stop_loss2={p['stop_loss2']}",
        "_EMA_Period2=2",
        f"_BB_Period2={p['BB_Period2']}",
        f"_BB_Dev2={p['BB_Dev2']}",
        "_RiskMode2=0",
    ])
    return (
        "[Common]\n"
...
def _extract_params(row):
    ...
    return dict(
        take_profit  = vi("param__take_profit",  12000),
        stop_loss    = vi("param__stop_loss",    8000),
        BB_Period1   = vi("param__BB_Period1",   20),
        BB_Dev1      = vf("param__BB_Dev1",      2.0),
        take_profit2 = vi("param__take_profit2", 16000),
        stop_loss2   = vi("param__stop_loss2",   10000),
        BB_Period2   = vi("param__BB_Period2",   20),
        BB_Dev2      = vf("param__BB_Dev2",      2.0),
    )
...
def _run_attempt(attempt, job_id, pd, parse_optimization_xml, write_passes, parse_forward_report):
    ...
    if xml_path.exists():
        ...
        print(f"\n── Attempt {attempt}/{MAX_ATTEMPTS}: joint genetic (1m OHLC, $10M, H1×H4) ──")
    ...
    et_key_cols = [c for c in [
        "param__take_profit", "param__stop_loss", "param__BB_Period1", "param__BB_Dev1",
        "param__take_profit2", "param__stop_loss2", "param__BB_Period2", "param__BB_Dev2",
    ...
    for rank, (_, row) in enumerate(top_n.iterrows(), 1):
        ...
        val_id = f"val_et_exp_h1xh4_{IS_TAG}_a{attempt}_{rank}"
        ...
        mark = "✔" if passed else "✖"
        print(f"  [{rank:2d}] {mark}  BB={p['BB_Period1']}/{p['BB_Period2']}  Dev={p['BB_Dev1']}/{p['BB_Dev2']}  "
    ...
    if passing:
        ...
        print(f"\n  → Selected: BB={sel[0]['BB_Period1']}/{sel[0]['BB_Period2']}  NP={sel[1]:+,.0f}  DD={sel[3]:.1f}%  Tr={sel[4]}")
    else:
        all_r = [(p, np_v, pf_v, dd_v, tr_v) for p, np_v, pf_v, dd_v, tr_v, _ in results]
        sel   = max(all_r, key=lambda x: x[1])
        print(f"\n  → No pass — fallback: BB={sel[0]['BB_Period1']}/{sel[0]['BB_Period2']}  NP={sel[1]:+,.0f}  DD={sel[3]:.1f}%  Tr={sel[4]}")
...
def main():
    ...
    print("=" * 68)
    print(f"  DT818_exp H1×H4 reopt  ({IS_TAG.upper()})")
    print(f"  IS: {IS_START} -> {IS_END}  S1=H1  S2=H4  EMA=2 fixed")
    print(f"  Sweep: TP/SL, BB period 10-50, BB dev 1.5-3.0")
    ...
    job_id = f"opti_exp_h1xh4_{IS_TAG}"
    ...
    print(f"\n{'=' * 68}")
    print(f"  DONE  —  DT818_exp  H1×H4  ({IS_TAG.upper()})")
    print(f"  S1 (H1): TP={p['take_profit']}  SL={p['stop_loss']}  BB={p['BB_Period1']}  Dev={p['BB_Dev1']}")
    print(f"  S2 (H4): TP={p['take_profit2']}  SL={p['stop_loss2']}  BB={p['BB_Period2']}  Dev={p['BB_Dev2']}")
    print(f"  ET {ET_RISK_PCT}% risk: NP={np_val:+,.0f}  PF={pf_val:.3f}  DD={dd_val:.1f}%  Trades={tr_val}")
    print(f"  Baseline (DT818_max M30×H4 @ 4%): NP=+$41,397  PF=2.090  DD=11.8%  Trades=88")
    print(f"{'=' * 68}")
