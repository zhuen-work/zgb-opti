"""DT818_3s full parameter reopt: S1=M30, S2=H4 (GetFractalValue entry).
...
OUTPUT_DIR    = Path(f"output/dt818_3s_m30xh4_{IS_TAG}")
...
def _run_attempt(attempt, job_id, pd, parse_optimization_xml, write_passes, parse_forward_report):
    ...
    for rank, (_, row) in enumerate(top_n.iterrows(), 1):
        ...
        val_id = f"val_et_3s_m30xh4_{IS_TAG}_a{attempt}_{rank}"
...
def main():
    ...
    print("=" * 65)
    print(f"  DT818_3s M30×H4 reopt  ({IS_TAG.upper()})")
    print(f"  IS: {IS_START} -> {IS_END}")
    print(f"  S1=M30  S2=H4  Chart={PERIOD}  EA=DT818_3s (GetFractalValue)")
    ...
    job_id = f"opti_3s_m30xh4_{IS_TAG}"
    ...
    print(f"\n{'=' * 65}")
    print(f"  DONE  —  DT818_3s  M30×H4  ({IS_TAG.upper()})")
