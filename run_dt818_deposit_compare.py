"""DT818 deposit size comparison: $5K vs $50K, 8-week IS (Jan 24 - Mar 21, 2026), risk 4%."""
...
def _build_ini(tester_inputs: str, report_id: str, deposit: int) -> str:
    return (
        "[Common]\n"
        f"Login={MT5_LOGIN}\nServer={MT5_SERVER}\nProxyEnable=0\nCertInstall=0\nNewsEnable=0\n"
        "\n[Charts]\n\n[Experts]\n\n[Tester]\n"
...
def _parse(report_path: Path, job_id: str) -> dict:
...
def main():
    ...
    print("=" * 62)
    print(f"  DT818 Deposit Comparison  |  Risk 4%  |  Every Tick")
    ...
    print(f"  {'Return %':<28}{ff(r5.get('return_pct', 0), '.1f')}{ff(r50.get('return_pct', 0), '.1f')}")
