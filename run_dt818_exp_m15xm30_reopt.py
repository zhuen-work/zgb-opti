"""DT818_exp reopt: S1=M15, S2=M30 (BB breakout strategy).
...
Chart period: M15 (finer of M15/M30).
"""
...
OUTPUT_DIR    = Path(f"output/dt818_exp_m15xm30_{IS_TAG}")
...
def _joint_set_lines():
    return [
        ...
        _sweep("_take_profit",  5000, 500,  1000, 15000),
        _sweep("_stop_loss",    3500, 500,  1000, 10000),
        _sweep("_EMA_Period1",  2,    1,    1,    5),
        _sweep("_BB_Period1",   20,   5,    10,   50),
        _sweep("_BB_Dev1",      "2.0", "0.5", "1.5", "3.0"),
        _fix("_RiskMode1", 0),
        _sweep("_Trade2",       1,    1,    0,    1),
        _fix("_time_frame2", TF2_ENUM),
        _sweep("_take_profit2", 6000, 500,  1000, 18000),
        _sweep("_stop_loss2",   4000, 500,  1000, 12000),
        _sweep("_EMA_Period2",  2,    1,    1,    5),
...
def _extract_params(row):
    ...
    return dict(
        take_profit  = vi("param__take_profit",  5000),
        stop_loss    = vi("param__stop_loss",    3500),
        EMA_Period1  = vi("param__EMA_Period1",  2),
        BB_Period1   = vi("param__BB_Period1",   20),
        BB_Dev1      = vf("param__BB_Dev1",      2.0),
        take_profit2 = vi("param__take_profit2", 6000),
        stop_loss2   = vi("param__stop_loss2",   4000),
        EMA_Period2  = vi("param__EMA_Period2",  2),
...
def _run_attempt(attempt, job_id, pd, parse_optimization_xml, write_passes, parse_forward_report):
    ...
    if xml_path.exists():
        ...
        print(f"\n── Attempt {attempt}/{MAX_ATTEMPTS}: joint genetic (1m OHLC, $10M, M15×M30) ──")
    ...
    et_key_cols = [c for c in [
        ...
        "param__BB_Period1", "param__BB_Dev1",
        "param__take_profit2", "param__stop_loss2", "param__EMA_Period2",
        "param__BB_Period2", "param__BB_Dev2",
    ...
    for rank, (_, row) in enumerate(top_n.iterrows(), 1):
        ...
        val_id = f"val_et_exp_m15xm30_{IS_TAG}_a{attempt}_{rank}"
        ...
        print(f"  [{rank:2d}] {mark}  EMA={p['EMA_Period1']}/{p['EMA_Period2']}  "
              f"BB={p['BB_Period1']}/{p['BB_Period2']}  Dev={p['BB_Dev1']}/{p['BB_Dev2']}  "
    ...
    if passing:
        ...
        print(f"\n  → Selected: EMA={sel[0]['EMA_Period1']}/{sel[0]['EMA_Period2']}  "
              f"BB={sel[0]['BB_Period1']}/{sel[0]['BB_Period2']}  Dev={sel[0]['BB_Dev1']}/{sel[0]['BB_Dev2']}  "
...
def main():
    ...
    print("=" * 68)
    print(f"  DT818_exp M15×M30 reopt  ({IS_TAG.upper()})")
    print(f"  IS: {IS_START} -> {IS_END}")
    print(f"  S1=M15  S2=M30  Chart={PERIOD}  EA=DT818_exp (BB breakout)")
    print(f"  Sweep: TP/SL, EMA 1-5, BB period 10-50, BB dev 1.5-3.0")
    ...
    job_id = f"opti_exp_m15xm30_{IS_TAG}"
    ...
    print(f"\n{'=' * 68}")
    print(f"  DONE  —  DT818_exp  M15×M30  ({IS_TAG.upper()})")
    print(f"  S1 (M15): TP={p['take_profit']}  SL={p['stop_loss']}  EMA={p['EMA_Period1']}  BB={p['BB_Period1']}  Dev={p['BB_Dev1']}")
    print(f"  S2 (M30): TP={p['take_profit2']}  SL={p['stop_loss2']}  EMA={p['EMA_Period2']}  BB={p['BB_Period2']}  Dev={p['BB_Dev2']}")
