...
from .sweep_rev_fast import simulate_fast as simulate


# Worker-global state, populated by _init_worker.
_W_TICKS: Optional[pd.DataFrame] = None
_W_M1: Optional[pd.DataFrame] = None
_W_M30: Optional[pd.DataFrame] = None
_W_META: Optional[SymbolMeta] = None


def _init_worker(symbol: str, start_iso: str, end_iso: str, meta_dict: dict, signal_tf: str):
    """Load ticks + M1 + signal-TF bars once per worker process.

    `signal_tf` is the TF string passed to load_bars (M30 for S1, H4 for S2).
    The loaded bars go into the global `_W_M30` slot regardless of TF — the
    sim engine only cares that they're the signal-TF bars.
    """
    global _W_TICKS, _W_M1, _W_M30, _W_META
    ...
    _W_M30 = load_bars(symbol, signal_tf, start, end)
    _W_META = SymbolMeta(**meta_dict)


def _run_job(payload: tuple) -> dict:
    """Run a single FBO Stream 1 sim."""
    job_id, cfg_dict, initial_balance = payload
    cfg = SweepRevConfig(**cfg_dict)
    out = dict(cfg_dict)
    out["job_id"] = job_id
    t0 = time.time()
    try:
        r = simulate(_W_TICKS, _W_M30, _W_M1, cfg, _W_META, initial_balance)
...
def _configs_hash(configs: list[SweepRevConfig]) -> str:
...
def run_sweep(
    ...
    `signal_tf` controls which TF bars get loaded by workers (M30 for S1,
    H4 for S2). Each cfg's `signal_tf_minutes` field must match this TF.
    """
