"""Parallel parameter sweep engine for the GMMA+RSI simulator.

Mirrors sweep_orb.py but operates on GMMAConfig and loads H1 + H4 bars.
"""
...
from .gmma import GMMAConfig, simulate


_W_TICKS: Optional[pd.DataFrame] = None
_W_H1: Optional[pd.DataFrame] = None
_W_H4: Optional[pd.DataFrame] = None
_W_META: Optional[SymbolMeta] = None


def _init_worker(symbol: str, start_iso: str, end_iso: str, meta_dict: dict):
    global _W_TICKS, _W_H1, _W_H4, _W_META
    ...
    _W_H1 = load_bars(symbol, "H1", start, end)
    _W_H4 = load_bars(symbol, "H4", start, end)
...
def _run_job(payload: tuple) -> dict:
    ...
    t0 = time.time()
    try:
        r = simulate(_W_TICKS, _W_H1, _W_H4, cfg, _W_META, initial_balance)
        ...
        for k in ("net_profit", "profit_factor", "drawdown_abs", "drawdown_pct",
            ...
            out[k] = np.nan
        out["trades"] = out["tp"] = out["sl"] = out["other"] = 0
...
def run_sweep(
    ...
    print(f"  [sweep {window_label}] done in {elapsed:.1f}s ({elapsed/len(configs):.2f}s/cfg), errors={n_err}")
...
def _progress(done: int, total: int, t0: float):
    ...
    msg = f"    [{done}/{total}] {rate:.2f} cfg/s  elapsed={elapsed:.0f}s  eta={eta:.0f}s"
    print(msg, file=sys.__stderr__ if sys.stdout != sys.__stdout__ else sys.stdout)
