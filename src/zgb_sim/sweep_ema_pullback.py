"""Parallel parameter sweep engine for EMA-pullback (Numba). Mirrors sweep_orb.py."""
...
from .ema_pullback_fast import simulate_fast as simulate
...
def _run_job(payload: tuple) -> dict:
    ...
    cfg = EMAPullbackConfig(**cfg_dict)
    out = dict(cfg_dict)
    out["job_id"] = job_id
    t0 = time.time()
    try:
...
def run_sweep(
    configs: list[EMAPullbackConfig],
    symbol: str,
