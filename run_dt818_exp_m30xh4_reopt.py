"""DT818_exp reopt: S1=M30, S2=H4 (BB breakout). EMA fixed at 2.
...
OUTPUT_DIR    = Path(f"output/dt818_exp_m30xh4_{IS_TAG}")
...
def _joint_set_lines():
    return [
        ...
        _sweep("_take_profit",  9000,  500,  3000,  20000),
        _sweep("_stop_loss",    7000,  500,  2000,  12000),
        _fix("_EMA_Period1", 2),
        ...
        _sweep("_take_profit2", 14000, 500,  4000,  28000),
        _sweep("_stop_loss2",   9000,  500,  3000,  15000),
        _fix("_EMA_Period2", 2),
...
def _extract_params(row):
    ...
    return dict(
        take_profit  = vi("param__take_profit",  9000),
        stop_loss    = vi("param__stop_loss",    7000),
        BB_Period1   = vi("param__BB_Period1",   20),
        BB_Dev1      = vf("param__BB_Dev1",      2.0),
        take_profit2 = vi("param__take_profit2", 14000),
        stop_loss2   = vi("param__stop_loss2",   9000),
...
def _run_attempt(attempt, job_id, pd, parse_optimization_xml, write_passes, parse_forward_report):
    ...
    for rank, (_, row) in enumerate(top_n.iterrows(), 1):
        ...
        val_id = f"val_et_exp_m30xh4_{IS_TAG}_a{attempt}_{rank}"
...
def main():
    ...
    print("=" * 68)
    print(f"  DT818_exp M30×H4 reopt  ({IS_TAG.upper()})")
    print(f"  IS: {IS_START} -> {IS_END}  S1=M30  S2=H4  EMA=2 fixed")
    ...
    job_id = f"opti_exp_m30xh4_{IS_TAG}"
    ...
    print(f"\n{'=' * 68}")
    print(f"  DONE  —  DT818_exp  M30×H4  ({IS_TAG.upper()})")
    print(f"  S1 (M30): TP={p['take_profit']}  SL={p['stop_loss']}  BB={p['BB_Period1']}  Dev={p['BB_Dev1']}")
