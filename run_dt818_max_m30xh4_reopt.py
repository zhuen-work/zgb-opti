"""DT818_max full parameter reopt: S1=M30, S2=H4.

Joint genetic sweep of all params, ET-validates top 10 distinct passes at 6% risk.
Retries up to 3x if no pass meets the gate (trades <= 150, NP >= $10K, DD <= 20%).

IS period: Jan 24 -> Mar 21, 2026 (same 8-week window).
Chart period: M30 (finer of M30/H4).
"""
...
ET_MAX_TRADES = 150
DD_THRESHOLD  = 20.0
ET_RISK_PCT   = 4.0     # risk % for ET validation
...
EA_PATH       = "DT818_max"
SYMBOL        = "XAUUSD"
PERIOD        = "M30"        # chart period (finer of M30/H4)
SPREAD        = 45
...
OUTPUT_DIR    = Path(f"output/dt818_max_m30xh4_{IS_TAG}")
...
def _build_val_ini(p, val_id):
    val_inputs = "\n".join([
        ...
        "_CapitalProtectionAmount=0.0", "_MaxEntries=1", "_LotMode=1", f"_RiskPct={ET_RISK_PCT}",
...
def _run_attempt(attempt, job_id, pd, parse_optimization_xml, write_passes, parse_forward_report):
    ...
    for rank, (_, row) in enumerate(top_n.iterrows(), 1):
        ...
        val_id = f"val_et_m30xh4_{IS_TAG}_a{attempt}_{rank}"
...
def main():
    ...
    print("=" * 65)
    print(f"  DT818_max M30×H4 reopt  ({IS_TAG.upper()})")
    print(f"  IS: {IS_START} -> {IS_END}")
    print(f"  S1=M30  S2=H4  Chart={PERIOD}")
    ...
    print(f"  Baseline (M30×H4 cross-tf @ 4%): NP=+$21,294  PF=2.110  DD=11.5%  Tr=88")
    print("=" * 65)

    job_id = f"opti_m30xh4_{IS_TAG}"
    ...
    print(f"\n{'=' * 65}")
    print(f"  DONE  —  M30×H4  ({IS_TAG.upper()})")
    ...
    print(f"  Baseline (cross-tf @ 4%): NP=+$21,294  PF=2.110  DD=11.5%  Trades=88")
    print(f"{'=' * 65}")
