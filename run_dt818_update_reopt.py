"""DT818_max_update full parameter reopt: S1=M30, S2=H4.

Same sweep space as DT818_max reopt — allows fair comparison of best params
for the always-update-pending variant.
"""
...
OUTPUT_DIR    = Path(f"output/dt818_update_m30xh4_{IS_TAG}")
...
def _run_attempt(attempt, job_id, pd, parse_optimization_xml, write_passes, parse_forward_report):
    ...
    for rank, (_, row) in enumerate(top_n.iterrows(), 1):
        ...
        val_id = f"val_et_update_{IS_TAG}_a{attempt}_{rank}"
...
def main():
    ...
    print("=" * 65)
    print(f"  DT818_max_update M30×H4 reopt  ({IS_TAG.upper()})")
    ...
    print(f"  DT818_max baseline @ 4%: NP=+$41,393  PF=2.090  DD=11.7%  Tr=88")
    print("=" * 65)

    job_id = f"opti_update_m30xh4_{IS_TAG}"
    ...
    print(f"\n{'=' * 65}")
    print(f"  DONE  —  DT818_max_update  M30×H4  ({IS_TAG.upper()})")
