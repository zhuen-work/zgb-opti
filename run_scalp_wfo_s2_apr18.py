"""Scalper v1 S2 (Asian session) Walk-Forward Optimization

S1 disabled. S2 swept for Asian session (1-8 UTC default).
...
OUTPUT_DIR    = Path("output/scalp_wfo_s2_m4_apr18")
SET_OUT       = Path("configs/sets/scalp_v1_s2_m4_apr18.set")
...
def _common_disabled_s1():
    """Disable S1, set S1 placeholders."""
    return [
        _fix("_Magic", 2000),
        _fix("_EntryTF", 5),
        "_BlockFriPM=true",
        ...
        _fix("_S1_RiskPct", "2.5"),
        _fix("_S1_DonchianBars", 20),
        _fix("_S1_TakeProfit", 150),
        _fix("_S1_StopLoss", 50),
        _fix("_S1_HalfTP_Ratio", "0.3"),
        _fix("_S1_PendingExpireBars", 2),
        _fix("_S1_TradeStartHour", 14),
        _fix("_S1_TradeEndHour", 22),
        _fix("_S1_DailyTargetPct", "5.0"),
        _fix("_S1_DailyLossPct", "10.0"),
...
def _base_params(donch, tp, sl, htp, tgt):
    """S2 fixed values (OOS). Hedge off, Loss fixed 12."""
    return _common_disabled_s1() + [
        "_S2_Enabled=true",
        "_S2_Comment=DT818_S2",
        _fix("_S2_RiskPct", RISK_PCT),
        _fix("_S2_DonchianBars", donch),
        _fix("_S2_TakeProfit", tp),
        _fix("_S2_StopLoss", sl),
        _fix("_S2_HalfTP_Ratio", htp),
        _fix("_S2_PendingExpireBars", 2),
        _fix("_S2_TradeStartHour", 1),
        _fix("_S2_TradeEndHour", 8),
        _fix("_S2_DailyTargetPct", tgt),
...
def _opt_params_sweep():
    """Narrowed S2 sweep: wider Donch only (20/25/30), hedge off, Loss fixed 12."""
    return _common_disabled_s1() + [
        "_S2_Enabled=true",
        "_S2_Comment=DT818_S2",
        _fix("_S2_RiskPct", RISK_PCT),
        _sweep("_S2_DonchianBars",   25, 5,  20, 30),
        _sweep("_S2_TakeProfit",     200, 50, 150, 200),
        _sweep("_S2_StopLoss",       50, 25, 50, 75),
        _sweep("_S2_HalfTP_Ratio",   0.3, 0.3, 0.3, 0.6),
        _fix("_S2_PendingExpireBars", 2),
        _fix("_S2_TradeStartHour", 1),
        _fix("_S2_TradeEndHour", 8),
        _sweep("_S2_DailyTargetPct", 6, 3, 3, 9),
        _fix("_S2_DailyLossPct",     12),
        _fix("_S2_DailyMaxWins", 0),
        _fix("_S2_DailyMaxLosses", 0),
        "_S2_HedgeMode=false",
...
def run_window_optimization(win_name, is_start, is_end):
    ...
    donch_c = [c for c in df.columns if "S2_DonchianBars" in c][0]
    tp_c    = [c for c in df.columns if "S2_TakeProfit" in c][0]
    sl_c    = [c for c in df.columns if "S2_StopLoss" in c][0]
    htp_c   = [c for c in df.columns if "S2_HalfTP_Ratio" in c][0]
    tgt_c   = [c for c in df.columns if "S2_DailyTargetPct" in c][0]
...
def main():
    ...
    # Dedupe by total IS NP and take top 5
    ...
    for i, (k, info) in enumerate(unique_robust, 1):
        ...
        print(f"\n  #{i} Donch={donch} TP={tp} SL={sl} HTP={htp} Tgt={tgt}  (in {info['count']} windows: {info['windows']})")
    ...
    for rank, s in enumerate(robust_scores[:5], 1):
        ...
        print(f"  {rank:<4}  {s['oos_total_np']:>+12,.0f}  {flag:>8}  Donch={p['donch']} TP={p['tp']} SL={p['sl']} HTP={p['htp']} Tgt={p['tgt']}")
