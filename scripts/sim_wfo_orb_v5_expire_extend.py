"""Boundary-extended expiry sweep for v5 (fractal_confirm forced on).

Prior WFO (output/wfo_orb_v5_may23/) showed Exp=360 dominates 6/8 top OOS
picks — but 360 was the GRID CEILING. This script reruns a focused subset
with expire extended to {240, 360, 480, 720, 1440} to test whether longer
pending life keeps improving NP/DD$ past 360, or plateaus / regresses.

Grid (focused on top-8 cluster):
  range_minutes = 90                       (unanimous in top-8)
  fixed_sl_pts  in {400, 550}              (top-8 spread)
  rr_ratio      in {2.0, 2.5, 3.0, 3.5, 4.0}  (top-8 spread)
  half_tp_ratio in {0.2, 0.4}              (top-8 modal)
  pending_expire in {240, 360, 480, 720, 1440}  (extends past prior 360 ceiling)
  fractal_confirm = True, fractal_width = 5

= 1 x 2 x 5 x 2 x 5 = 100 configs per window x 4 windows = 400 sims.
ETA ~6-12 min total.
"""
...
N_WORKERS = 6
SIGNAL_TF = "M5"
OUT_DIR = ROOT / "output" / "wfo_orb_v5_expire_extend"
...
def build_grid() -> list[ORBConfig]:
    grid = []
    for range_min in (90,):
        for fixed_sl in (400, 550):
            for rr in (2.0, 2.5, 3.0, 3.5, 4.0):
                for htp in (0.2, 0.4):
                    for expire_min in (240, 360, 480, 720, 1440):
...
def select_robust(per_window, top_n=20, max_candidates=15):
    counts = {}
...
def fmt_cfg(c: ORBConfig) -> str:
    return (f"SL={c.fixed_sl_pts:>4} RR={c.rr_ratio:<3} "
            f"HTP={c.half_tp_ratio:<3} Exp={c.pending_expire_minutes:>4}")


def main():
    print(f"[BOOT] v5 expire-extend sweep on {WINDOWS[0][0]}..{WINDOWS[-1][0]}", flush=True)
    ...
    cands = select_robust(is_per, top_n=20, max_candidates=15)
    ...
    for i, info in enumerate(ranked[:10]):
        ...
        print(f"  rank#{i+1} prof={prof}/4 total_np=${total_np:>+8,.0f} "
              f"NP/DD$={np_dd:>6.1f}  {fmt_cfg(c)}", flush=True)

    # Aggregate: expire-value summary regardless of other params
    print(f"\n[SUMMARY] OOS NP/DD$ by expire (averaged across top-10 candidates):", flush=True)
    by_exp = {}
    for info in ranked:
        e = info["cfg"].pending_expire_minutes
        by_exp.setdefault(e, []).append(info.get("np_dd_ratio", 0))
    for e in sorted(by_exp.keys()):
        vals = by_exp[e]
        avg = sum(vals) / len(vals) if vals else 0
        print(f"  Exp={e:>4}: n={len(vals)}  avg NP/DD$={avg:>7.1f}  best={max(vals):>7.1f}", flush=True)

    rows = []
    for i, info in enumerate(ranked):
        c = info["cfg"]
        rows.append({
            "rank": i + 1, "prof_count": info.get("prof_count", 0),
            "total_np": info.get("total_np", 0),
