"""WFO for FBO Stream 1 only (FBO_FVG_v2 EA), at spread=60.

Same Phase A/B/C/D structure as scalper WFO scripts:
  - Phase A: IS sweep on each of 3 rolling windows (4w IS / 2w OOS, ending Apr 25)
  - Phase B: top-30 robust filter, deduped, max 15 candidates
  - Phase C: OOS sweep with candidates
  - Phase D: rank by (prof_count desc, NP/AvgDD desc), write winner setfile

FBO conventions (per memory):
  - $10k deposit, 3% risk per trade
  - Spread = 60 pts (set globally in tick_loader.SIM_SPREAD_PTS)

Initial exploratory grid: 108 combos. Sized to finish in ~20 min so the user
can iterate. Wider grid is the next step if results are promising.
"""
...
RISK_PCT = 3.0
DEPOSIT = 10_000.0
N_WORKERS = 4   # Conservative: 4 workers × ~750 MB resident = ~3 GB. With
                # Numba the JIT runtime is short (~5s/cfg) but peak load
                # memory at init is ~1.5 GB/worker, so 8 workers risks OOM.
...
OUT_DIR = ROOT / "output" / "sim_wfo_fbo_s1_spread60_apr25"
SET_OUT = ROOT / "configs" / "sets" / "scalp_v1_sim_fbo_s1_spread60_apr25.set"


def build_config_grid(tiny: bool = False) -> list[FBOS1Config]:
    """Initial exploratory grid for FBO Stream 1 at spread=60.

    Reference (from fbo_v2_3pct_m30xh4xh1_apr11_reopt_10k.set, NP/DD=1300):
      Bars=8, TP=25000, SL=5000, HTP=0.3, SMA=5, PEB=2.

    Sweep around it: TP 15000-30000, SL 3000-10000, Bars 4-10, HTP 0/0.3/0.6,
    SMA 5/10/20.
    """
    if tiny:
        bars_vals = (8,)
        ...
        bars_vals = (4, 6, 8, 10)            # 4
        tp_vals = (15_000, 20_000, 25_000)   # 3
        sl_vals = (3_000, 5_000, 10_000)     # 3
        htp_vals = (0.0, 0.3, 0.6)           # 3
        sma_vals = (5, 10, 20)               # 3
...
def write_setfile(cfg: FBOS1Config, path: Path):
    ...
    lines = [
        ...
        f"_FBO1=1||1||1||1||1||N",
        f"_OrderComment=FBO_A",
        f"_time_frame=30||30||1||30||30||N",
        f"_take_profit={c['take_profit_pts']}||{c['take_profit_pts']}||1||{c['take_profit_pts']}||{c['take_profit_pts']}||N",
        f"_stop_loss={c['stop_loss_pts']}||{c['stop_loss_pts']}||1||{c['stop_loss_pts']}||{c['stop_loss_pts']}||N",
        f"_Bars={c['fractal_bars']}||{c['fractal_bars']}||1||{c['fractal_bars']}||{c['fractal_bars']}||N",
        f"_EMA_Period1={c['sma_period']}||{c['sma_period']}||1||{c['sma_period']}||{c['sma_period']}||N",
        f"_HalfTP1={c['half_tp_ratio']}||{c['half_tp_ratio']}||1||{c['half_tp_ratio']}||{c['half_tp_ratio']}||N",
        f"_FBO2=0||0||1||0||0||N",
        f"_FBO3=0||0||1||0||0||N",
        f"_FVG1=0||0||1||0||0||N",
        f"_FVG2=0||0||1||0||0||N",
...
def sanity_et(cfg: FBOS1Config, meta: SymbolMeta):
    ...
    r = simulate(ticks, m30, m1, cfg, meta, initial_balance=DEPOSIT,
...
def main():
    ...
    try:
        ...
        print("=" * 72)
        mode = "TINY (1 cfg)" if args.tiny else "EXPLORATORY (108 combos)"
        print(f"  PYSIM WFO FBO Stream 1 (Apr 25, spread=60) — sim IS+OOS   [{mode}]")
        print("=" * 72)
        ...
        print("\n  Pre-warming tick/M1/M30 cache for full range...")
        t0 = time.time()
        ...
        _ = load_bars(SYMBOL, "M30", _to_utc(PREWARM_START), _to_utc(PREWARM_END))
