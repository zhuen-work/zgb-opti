"""WFO for FVG Stream 2 only (FBO_FVG_v2 EA, H4 timeframe), at spread=60.

Same grid as FVG S1 but signal TF = H4. PEB units in H4-bars.
...
OUT_DIR = ROOT / "output" / "sim_wfo_fvg_s2_spread60_apr25"
SET_OUT = ROOT / "configs" / "sets" / "fvg_s2_sim_spread60_apr25.set"
SIGNAL_TF = "H4"
...
def write_setfile(cfg: FVGConfig, path: Path):
    """Write FVG S2 winner setfile (FBO + FVG S1 disabled)."""
    c = asdict(cfg)
    lines = [
        ...
        f"_FVG_TF2=16388||16388||1||16388||16388||N",   # H4
        f"_FVG_MinSize2={c['min_size_pts']}||{c['min_size_pts']}||1||{c['min_size_pts']}||{c['min_size_pts']}||N",
        f"_FVG_MaxAge2={c['max_age_bars']}||{c['max_age_bars']}||1||{c['max_age_bars']}||{c['max_age_bars']}||N",
        f"_MaxZones2={c['max_zones']}||{c['max_zones']}||1||{c['max_zones']}||{c['max_zones']}||N",
        f"_RR_Ratio2={c['rr_ratio']}||{c['rr_ratio']}||1||{c['rr_ratio']}||{c['rr_ratio']}||N",
        f"_SL_Buffer2={c['sl_buffer_pts']}||{c['sl_buffer_pts']}||1||{c['sl_buffer_pts']}||{c['sl_buffer_pts']}||N",
        f"_PendingExpireBars_F2={c['pending_expire_bars']}||{c['pending_expire_bars']}||1||{c['pending_expire_bars']}||{c['pending_expire_bars']}||N",
        f"_HalfTP_F2={c['half_tp_ratio']}||{c['half_tp_ratio']}||1||{c['half_tp_ratio']}||{c['half_tp_ratio']}||N",
    ]
...
def main():
    ...
    try:
        ...
        print("=" * 72)
        mode = "TINY" if args.tiny else "FVG S2 H4 (288 combos)"
        print(f"  PYSIM WFO FVG Stream 2 (H4, Apr 25, spread=60)   [{mode}]")
        print("=" * 72)
