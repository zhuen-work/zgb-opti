"""Update forward_projection.json + console for v6 (parents + STOP-ext hedge).

Extends update_v5_projection.py: adds STOP-on-extension hedge layer on top
of each parent stream's SL events. Hedge contribution is deal-merged into
portfolio before computing weekly distribution + per-stream slopes.

Hedge config (uniform across all 6 streams, from WFO winner):
  ext_pts=100, tp_mult=3.0, sl_mult=1.0, expire_min=240, f1_sec=1800

Usage:
  python scripts/update_v6_projection.py             # update + push
  python scripts/update_v6_projection.py --no-push   # update only
  python scripts/update_v6_projection.py --dry-run   # print, don't write
"""
...
# v6 STOP-ext hedge (uniform across all 6 streams)
...
def main():
    ...
    setfile_name = f"dt818_pro_v6_{setfile_label}_may23_may16.set"
    ...
    per_stream_deals = {}    # stream -> list of (ts, pnl) -- PARENT + HEDGE merged
    per_stream_trades = {}
    per_stream_np = {}
    ...
    for stream in ("S1","S2","S3","S4","S5","S6"):
        ...
        # v6: add STOP-ext hedge deals
        sl_ev = extract_sl_events_fmt(r.deals)
        hedge_pnls_ns = simulate_stop_extension_hedges(sl_ev, ticks_arr, STREAM_CFGS[stream], STOPEXT_CFG)
        # hedge sim returns ts_ns; convert to tz-naive pd.Timestamp to match parent format
        ...
        # Merge parent + hedge into per-stream deals (used downstream for weekly distribution)
        merged = parent_deals + hedge_deals
        per_stream_deals[stream] = merged
        per_stream_trades[stream] = len(merged)
        per_stream_np[stream] = sum(p for _, p in merged)
        per_stream_parent_np[stream] = parent_np
        per_stream_hedge_np[stream] = hedge_np
        per_stream_hedge_n[stream] = hedge_n
        per_stream_hedge_wr[stream] = round(hedge_wins / hedge_n * 100, 1) if hedge_n else 0.0
    ...
    weekly_live = {
        ...
        # v6 split: parent share + hedge share based on sim ratio
    ...
    proj["method"] = "view_c_decay_adjusted_v6_parents_plus_stopext"
    ...
    proj[raw_key] = {
        "v6_combined_np": round(portfolio_np, 0),
        "v6_parent_np": round(parent_total, 0),
        "v6_hedge_np": round(hedge_total, 0),
        "v6_hedge_share_pct": round(hedge_total / max(portfolio_np, 1) * 100, 1),
        "v6_dd_pct": round(dd_pct, 2),
        "v6_ndd": round(ndd, 2),
        "v6_pf": round(pf, 2),
        "v6_trades": len(all_deals),
        "v6_hedge_n": sum(per_stream_hedge_n.values()),
        "compound_weekly_rate_v6": compound_weekly_rate,
    }
    ...
    for k in list(proj.keys()):
        if k.startswith("per_stream_sim_contribution_") and k != contrib_key:
            del proj[k]
    # Add v6 hedge cfg block (single uniform config across all 6 streams)
    ...
    proj["ea"] = "DT818_pro_v6.mq5"
    proj["ea_version"] = "v6 V2 fractal-confirmed entry + STOP-on-extension hedge (MAY23 reopt 2026-05-24)"
    ...
    notes.append(
        f"2026-05-24 PT2: v6 setfile (parents v5 + STOP-on-extension hedge). "
        f"STOP-ext params: ext_pts=100, tp_mult=3.0, sl_mult=1.0 (uniform across S1-S6). "
        f"Sim {sim_days}d at {PER_STREAM_RISK}%/stream {SPREAD}pt: "
        f"parent NP=${parent_total:+,.0f} + hedge NP=${hedge_total:+,.0f} = combined ${portfolio_np:+,.0f} NP/DD$={ndd:.2f}. "
        f"Hedge contributes {hedge_total/max(portfolio_np,1)*100:.1f}% of total. "
        f"Method: view_c_decay_adjusted_v6_parents_plus_stopext."
    )
    proj["notes"] = notes[-10:]
    proj["wfo_log_source"] = str(ROOT / "output" / "wfo_hedge_geometry_may23" / "geometry_sweep.csv")
