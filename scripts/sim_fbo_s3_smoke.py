"""Smoke test FBO Stream 3 (H1) sim with reference params at spread=60."""
...
def main() -> int:
    # Reference S3 params from fbo_v2_3pct_m30xh4xh1_apr11_reopt_10k.set
    cfg = FBOS1Config(
        risk_pct=3.0,
        fractal_bars=4,
    ...
    print("=" * 72)
    print("  FBO Stream 3 (H1) smoke test (spread=60, $10k)")
    print(f"  Cfg: TF=H1 Bars={cfg.fractal_bars} TP={cfg.take_profit_pts} "
    ...
    try:
        ...
        h1 = load_bars("XAUUSD", "H1", start, end)
        print(f"  Loaded ticks={len(ticks):,}  M1={len(m1):,}  H1={len(h1):,}")

        result = simulate_fast(ticks, h1, m1, cfg, meta, initial_balance=10_000.0)
