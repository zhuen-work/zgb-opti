"""V5 Follow-up: Slow-MA Neighbor Sweep around s8_21 (5 slower-MA candidates).

Run:  $env:PYTHONPATH = "src"; python scripts/sim_orb_sma_cross_slow_ma_sweep.py

Outputs:
  docs/reports/2026-05-25-sma-cross-slow-ma-sweep.md
"""
...
REPORT  = ROOT / "docs" / "reports" / "2026-05-25-sma-cross-slow-ma-sweep.md"
DEPOSIT = 10_000.0
SPREAD  = 30
RISK_PER_STREAM = 1.5
...
def main() -> int:
        ...
        for sidx, s in enumerate(streams, start=1):
            for mode_name, on, fast, slow in MODES:
                try:
                    ...
                    print(
                        f"  {wname} S{sidx} {mode_name:8s}: net={r.net_profit:+.2f}"
    ...
    with REPORT.open("w", encoding="utf-8") as f:
        f.write("# V5 Follow-up: Slow-MA Neighbor Sweep around s8_21\n\n")
        f.write(
    ...
    if candidates:
        ...
        print("\nVerdict: REJECT -- no pair clears the bar")
