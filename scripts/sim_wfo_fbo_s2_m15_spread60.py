"""WFO for FBO Stream 2 ALTERNATIVE — M15 timeframe instead of default H4.

User experiment: maybe S2 was unrobust at H4 due to too few signals (H4 = 174
bars over 6 weeks). M15 has ~16x more bars (~2790), giving more entry chances
and finer fractal resolution.

Note: M15 isn't a default in MT5 enum but is a valid TIMEFRAME_M15 value.
Pending expiry of N M15 bars = N × 15 minutes.
...
OUT_DIR = ROOT / "output" / "sim_wfo_fbo_s2_m15_spread60_apr25"
SET_OUT = ROOT / "configs" / "sets" / "fbo_s2_m15_sim_spread60_apr25.set"
SIGNAL_TF = "M15"
SIGNAL_TF_MINUTES = 15


def build_config_grid(tiny: bool = False) -> list[FBOS1Config]:
    """S2-M15 exploratory grid at spread=60.

    No EA reference for M15 — picked sane values for a 15-min timeframe:
      - TP 4-12k (smaller moves than H1/H4)
      - SL 2-6k (tight relative to TP)
      - SMA 10-50 (15min × 50 = 12.5hr lookback)
      - PEB 2-6 bars (30min - 1.5hr)
    """
    if tiny:
        bars_vals = (6,)
        tp_vals = (8_000,)
        ...
        bars_vals = (4, 6, 8)                  # 3
        tp_vals = (4_000, 8_000, 12_000)       # 3 — M15 sees smaller moves
        sl_vals = (2_000, 4_000, 6_000)        # 3
        htp_vals = (0.0, 0.3, 0.6)             # 3
        sma_vals = (10, 25, 50)                # 3
        peb_vals = (2, 4, 6)                   # 3 — M15: 30min, 1hr, 1.5hr
...
def write_setfile(cfg: FBOS1Config, path: Path):
    """Write S2-M15 winner setfile (FBO Stream 2 with M15 timeframe)."""
    c = asdict(cfg)
    lines = [
        ...
        f"_time_frame2=15||15||1||15||15||N",   # M15
...
def main():
    ...
    try:
        ...
        print("=" * 72)
        mode = "TINY (1 cfg)" if args.tiny else "S2-M15 (729 combos)"
        print(f"  PYSIM WFO FBO Stream 2 ALT (M15, Apr 25, spread=60) — sim IS+OOS   [{mode}]")
        print("=" * 72)
