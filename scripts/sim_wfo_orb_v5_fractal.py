"""Full WFO for v5 EA with fractal_confirm forced ON (width=5).

Mirrors the standard sim_wfo_orb.py Phase 1 grid (1125 entry configs) but
forces fractal_confirm=True, fractal_width=5 for every config so the
optimum is found ASSUMING the V2 gate is enabled — i.e. it answers
"if we ship v5 with fractal on, what are the best ORB params?"

Grid: range × sl × rr × htp × expire = 3×5×5×5×3 = 1125 per window.
Windows: WINDOWS_MAY23 (4 IS/OOS folds; W4 IS ends 2026-05-16, OOS 05-23).

Output: output/wfo_orb_v5_may23/{p1_is_W{1-4}.parquet, p1_oos_W{1-4}.parquet,
oos_rank.csv}.

Markers for Monitor: [BOOT], [GRID], [WIN], [OOS], [RANK], [DONE].
"""
...
N_WORKERS = 6
SIGNAL_TF = "M5"
OUT_DIR = ROOT / "output" / "wfo_orb_v5_may23"
...
def build_grid() -> list[ORBConfig]:
    """Full standard Phase 1 grid with V2 forced ON (width=5)."""
    grid = []
    for range_min in (60, 90, 120):                            # 3
        for fixed_sl in (400, 550, 700, 850, 1000):            # 5
            for rr in (2.0, 2.5, 3.0, 3.5, 4.0):               # 5
                for htp in (0.0, 0.2, 0.4, 0.6, 0.8):          # 5
                    for expire_min in (120, 240, 360):         # 3
...
def fmt_cfg(c: ORBConfig) -> str:
    return (f"Range={c.range_minutes:>3} SL={c.fixed_sl_pts:>4} "
            ...
            f"Exp={c.pending_expire_minutes:>3}")


def main():
    print(f"[BOOT] v5-fractal WFO on {WINDOWS[0][0]}..{WINDOWS[-1][0]} (fractal_confirm=True, width=5)", flush=True)
    m = symbol_meta(SYMBOL)
    meta = SymbolMeta(point=m["point"], digits=m["digits"], tick_size=m["tick_size"],
                      tick_value=m["tick_value"], stops_level_pts=m["stops_level"],
                      volume_min=m["volume_min"], volume_max=m["volume_max"],
                      volume_step=m["volume_step"])
    # MT5 stays alive — workers may need to fetch any month not yet cached.
