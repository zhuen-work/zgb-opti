...
Run:  $env:PYTHONPATH = "src"; python scripts/sim_orb_ma7_trail_ab.py

Outputs:
  docs/reports/2026-05-25-ma7-trail-ab.md  — comparison table + verdict
...
def main() -> int:
    ...
    for wname, wstart, wend in WINDOWS:
        ...
        for sidx, s in enumerate(streams, start=1):
            for mode_name, ma in [("off", False), ("on", True)]:
                try:
                    r = simulate_fast(ticks, m5, m1, _build_cfg(s, ma), meta, DEPOSIT)
                    net_hc, dd_hc, nd_hc = _haircut_np_dd(r.net_profit, r.max_drawdown)
                    rows.append({
                        "window": wname,
                        "days": days,
                        "stream": f"S{sidx}",
    ...
    with REPORT.open("w", encoding="utf-8") as f:
        ...
        if wins >= 3 and regressions == 0:
            verdict = "**Verdict: ADVANCE** — ≥3/4 windows improved ≥+10% with zero regressions. Promote `ma_trail=on` as a WFO dim."
        elif regressions > 0:
            verdict = "**Verdict: REJECT** — regime-dependent regression observed (≤−15% in ≥1 window)."
        else:
            verdict = "**Verdict: ITERATE** — mixed results; revisit MA period / buffer / activation rule."
        f.write(f"- {verdict}\n")
