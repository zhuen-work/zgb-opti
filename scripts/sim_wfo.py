...
OUT_DIR = ROOT / "output" / "sim_wfo_apr25"
SET_OUT = ROOT / "configs" / "sets" / "scalp_v1_sim_apr25.set"


def build_config_grid(tiny: bool = False, hedge: bool = False) -> list[S1Config]:
    """Initial sweep spec from plan. tiny=True produces a small sanity grid."""
    if tiny:
        donch_vals = (20, 25)
        tp_vals = (150, 200)
        sl_vals = (50,)
...
def main():
    ...
    if args.hedge:
        OUT_DIR = ROOT / "output" / "sim_wfo_hedge_apr25"
        SET_OUT = ROOT / "configs" / "sets" / "scalp_v1_sim_hedge_apr25.set"
    ...
    try:
        ...
        print(f"\n  {'Rank':<5}{'Total OOS NP':>14}{'ROI%':>8}{'AvgDD%':>8}{'NP/DD':>8}{'AllProf':>9}  Params")
