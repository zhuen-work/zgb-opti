...
Flow:
  1. Build config grid (925 combos: 900 narrow + 25 HTP=0.9 outlier).
...
OUT_DIR = ROOT / "output" / "sim_wfo_spread60_narrow_apr25"
SET_OUT = ROOT / "configs" / "sets" / "scalp_v1_sim_spread60_narrow_apr25.set"


def build_config_grid(tiny: bool = False, hedge: bool = False) -> list[S1Config]:
    """Narrowed sweep at 60-pt spread, HTP=0.

    Grid trimmed to the parameter values that covered >=92% of the deep-run
    W1 top-100 by NP/DD: dropped Donch=30, TP=300/500/700, SL=100/150 because
    each contributed <=6% individually.

    Bonus: 25 extra configs at HTP=0.9 on (Donch=50, TP=2000, SL=500) — this
    one cluster showed a strong NP/DD jump in the dedicated HTP test
    (base #2: 22.68 -> 39.48 with HTP=0.9). Keeps that pattern in candidates
    without the 5x grid cost of a global HTP sweep.
    """
    if tiny:
        donch_vals = (50,)
        tp_vals = (1500,)
        ...
        donch_vals = (40, 50, 60)
        tp_vals = (1000, 1500, 2000)
        sl_vals = (70, 200, 300, 500)
        htp_vals = (0.0,)
        tgt_vals = (12.0, 18.0, 25.0, 35.0, 50.0)
        loss_vals = (4.0, 6.0, 8.0, 12.0, 18.0)
        peb_vals = (2,)
    grid: list[S1Config] = []
    ...
    # HTP=0.9 outlier patch on (Donch=50, TP=2000, SL=500) cluster
    if not tiny:
        for tgt in (12.0, 18.0, 25.0, 35.0, 50.0):
            for loss in (4.0, 6.0, 8.0, 12.0, 18.0):
                grid.append(S1Config(
                    risk_pct=RISK_PCT,
                    donchian_bars=50,
                    take_profit_pts=2000,
                    stop_loss_pts=500,
                    half_tp_ratio=0.9,
...
def main():
    ...
    if args.tiny:
        OUT_DIR = ROOT / "output" / "sim_wfo_spread60_narrow_tiny"
        SET_OUT = ROOT / "configs" / "sets" / "scalp_v1_sim_spread60_narrow_tiny.set"

    try:
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        print("=" * 72)
        mode = "TINY (sanity)" if args.tiny else "NARROW (925 combos)"
        print(f"  PYSIM WFO NARROW (Apr 25, spread=60) — 3 windows, sim IS+OOS   [{mode}]")
        print("=" * 72)
