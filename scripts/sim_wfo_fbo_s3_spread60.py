"""WFO for FBO Stream 3 only (FBO_FVG_v2 EA, H1 timeframe), at spread=60.

Mirrors S1/S2 WFO; differs in:
  - signal TF = H1 (60 min)
  - SMA periods 10/25/50 (between S1 and S2)
  - PEB swept in H1-bar units (1 = 1hr, 2 = 2hr, 3 = 3hr)

Reference (S3 in fbo_v2_3pct_m30xh4xh1_apr11_reopt_10k.set):
  Bars=4, TP=25000, SL=6000, SMA=25, HTP=0.4, PEB=2.
...
OUT_DIR = ROOT / "output" / "sim_wfo_fbo_s3_spread60_apr25"
SET_OUT = ROOT / "configs" / "sets" / "fbo_s3_sim_spread60_apr25.set"
SIGNAL_TF = "H1"
SIGNAL_TF_MINUTES = 60


def build_config_grid(tiny: bool = False) -> list[FBOS1Config]:
    """S3 (H1) exploratory grid at spread=60.

    Reference (S3 in fbo_v2_3pct_m30xh4xh1_apr11_reopt_10k.set):
      Bars=4, TP=25000, SL=6000, SMA=25, HTP=0.4, PEB=2.

    Grid 729: Bars 4/6/8, TP 12/18/25k, SL 5/8/10k, HTP 0/0.3/0.6,
    SMA 10/25/50, PEB 1/2/3 (H1-bar units).
    """
    if tiny:
        bars_vals = (4,)
        tp_vals = (25_000,)
        ...
        bars_vals = (4, 6, 8)                  # 3
        tp_vals = (12_000, 18_000, 25_000)     # 3 — span around H1 reference
        sl_vals = (5_000, 8_000, 10_000)       # 3
        htp_vals = (0.0, 0.3, 0.6)             # 3
        sma_vals = (10, 25, 50)                # 3
        peb_vals = (1, 2, 3)                   # 3 — H1 PEB (1/2/3 hr)
...
def write_setfile(cfg: FBOS1Config, path: Path):
    """Write FBO Stream 3 winner setfile (S1+S2+FVG disabled)."""
    c = asdict(cfg)
    lines = [
        ...
        f"_FBO2=0||0||1||0||0||N",
        f"_FBO3=1||1||1||1||1||N",
        f"_OrderComment3=FBO_C",
        f"_time_frame3=16385||16385||1||16385||16385||N",   # H1
        f"_take_profit3={c['take_profit_pts']}||{c['take_profit_pts']}||1||{c['take_profit_pts']}||{c['take_profit_pts']}||N",
        f"_stop_loss3={c['stop_loss_pts']}||{c['stop_loss_pts']}||1||{c['stop_loss_pts']}||{c['stop_loss_pts']}||N",
        f"_Bars3={c['fractal_bars']}||{c['fractal_bars']}||1||{c['fractal_bars']}||{c['fractal_bars']}||N",
        f"_EMA_Period3={c['sma_period']}||{c['sma_period']}||1||{c['sma_period']}||{c['sma_period']}||N",
        f"_HalfTP3={c['half_tp_ratio']}||{c['half_tp_ratio']}||1||{c['half_tp_ratio']}||{c['half_tp_ratio']}||N",
        f"_FVG1=0||0||1||0||0||N",
        f"_FVG2=0||0||1||0||0||N",
    ]
...
def main():
    ...
    try:
        ...
        print("=" * 72)
        mode = "TINY (1 cfg)" if args.tiny else "S3 H1 (729 combos)"
        print(f"  PYSIM WFO FBO Stream 3 (H1, Apr 25, spread=60) — sim IS+OOS   [{mode}]")
        print("=" * 72)
