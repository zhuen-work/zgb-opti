//+------------------------------------------------------------------+
//| DT818_pro_v2.1_h.mq5                                              |
//| ORB EA: 6-stream rank-portfolio + 6 hedge RETRY sub-streams       |
//|                                                                   |
//| Hedge variant of v2.1 (inherits both timezone fixes).             |
//| For each parent stream S1..S6, a hedge sub-stream S1h..S6h        |
//| RETRIES the parent's original pending after the parent SL hits.   |
//|                                                                   |
//| RETRY HEDGE (different from old DT818_pro_hedge.mq5):             |
//|   Old: LIMIT in OPPOSITE direction at sl_price ± buffer           |
//|        (reversion play — bet on price BOUNCING BACK after SL).    |
//|   NEW: STOP in SAME direction at PARENT'S ORIGINAL entry price,   |
//|        with PARENT'S ORIGINAL SL/TP/lots                          |
//|        (retry play — bet that the original breakout signal is     |
//|         still valid; the first attempt got stopped but a second   |
//|         entry into the same setup may work).                      |
//|                                                                   |
//|   Parent BUY  position SL'd → place BUY_STOP at original parent   |
//|                               entry price (above current price    |
//|                               since parent SL'd below entry)      |
//|   Parent SELL position SL'd → place SELL_STOP at original parent  |
//|                               entry price (below current price    |
//|                               since parent SL'd above entry)      |
//|                                                                   |
//|   Same entry, same SL, same TP, same lots as parent's original.   |
//|                                                                   |
//| Hedge magic numbers: 7111/7222/7333/7444/7555/7666 (parents are   |
//| 1111-6666; old hedge EA's 5111-5333 would now collide with parent |
//| S5 magic 5555 so we shifted to 7xxx for the 6-stream layout).     |
//|                                                                   |
...
//====================================================================
// HEDGE_S1h — hedge sub-stream for ORB_S1 (magic 7111)
// Fires SELL_STOP / BUY_STOP at parent SL price ± buffer (continuation play)
//====================================================================
input bool    _HEDGE_S1_Enabled                = true;
input int     _HEDGE_S1_Magic                  = 7111;
input string  _HEDGE_S1_Comment                = "ORB_S1h";
input int     _HEDGE_S1_ParentMagic            = 1111;
input int     _HEDGE_S1_BufferPts              = 100;
...
//====================================================================
// HEDGE_S2h — hedge sub-stream for ORB_S2 (magic 7222)
//====================================================================
input bool    _HEDGE_S2_Enabled                = true;
input int     _HEDGE_S2_Magic                  = 7222;
...
//====================================================================
// HEDGE_S3h — hedge sub-stream for ORB_S3 (magic 7333)
//====================================================================
input bool    _HEDGE_S3_Enabled                = true;
input int     _HEDGE_S3_Magic                  = 7333;
...
//====================================================================
// HEDGE_S4h — hedge sub-stream for ORB_S4 (magic 7444)
//====================================================================
input bool    _HEDGE_S4_Enabled                = true;
input int     _HEDGE_S4_Magic                  = 7444;
input string  _HEDGE_S4_Comment                = "ORB_S4h";
input int     _HEDGE_S4_ParentMagic            = 4444;
input int     _HEDGE_S4_BufferPts              = 100;
...
//====================================================================
// HEDGE_S5h — hedge sub-stream for ORB_S5 (magic 7555)
//====================================================================
input bool    _HEDGE_S5_Enabled                = true;
input int     _HEDGE_S5_Magic                  = 7555;
input string  _HEDGE_S5_Comment                = "ORB_S5h";
input int     _HEDGE_S5_ParentMagic            = 5555;
input int     _HEDGE_S5_BufferPts              = 100;
...
//====================================================================
// HEDGE_S6h — hedge sub-stream for ORB_S6 (magic 7666)
//====================================================================
input bool    _HEDGE_S6_Enabled                = true;
input int     _HEDGE_S6_Magic                  = 7666;
input string  _HEDGE_S6_Comment                = "ORB_S6h";
input int     _HEDGE_S6_ParentMagic            = 6666;
input int     _HEDGE_S6_BufferPts              = 100;
...
//====================================================================
// ProcessHedgeStream — RETRY parent's original pending after parent SL hit.
//
// Logic: when parent SL hits, look up parent's IN deal to find the original
// pending order. Place a NEW pending of the SAME type with the SAME entry,
// SAME SL, SAME TP, SAME lots. Bet: original breakout signal still valid,
// give it a second chance.
//
//   Parent BUY  position SL'd → BUY_STOP  at original parent entry (above current price)
//   Parent SELL position SL'd → SELL_STOP at original parent entry (below current price)
//
// F1 filter (MaxSecondsAfterEntry): skip retry if parent SL hit too LATE
// after entry. Fast SL = pure overshoot (good retry candidate). Slow SL =
// genuine trend reversal (bad retry candidate).
//
...
void ProcessHedgeStream(HedgeStreamCfg &hcfg)
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      // Find IN deal for this position
      ...
      {
         HedgeMarkSeen(dt);
         PrintFormat("[%s] hedge SKIPPED: no IN deal found for position %I64d",
                     hcfg.comment, pos_id);
         continue;
      }
      ...
      // Apply tp_mult: scale parent's TP distance by configured multiplier.
      //   tp_mult = 1.0 → use parent's exact TP (default behavior)
      //   tp_mult = 0.75 → retry TP at 75% of parent's TP distance (closer, easier to hit)
      //   tp_mult = 0.5 → half the parent's TP distance
      double retry_tp;
      if(orig_type == ORDER_TYPE_BUY_STOP)
      {
         double orig_tp_dist = orig_tp - orig_entry;   // positive for BUY
         retry_tp = NormPrice(orig_entry + orig_tp_dist * hcfg.tp_mult);
      }
      else if(orig_type == ORDER_TYPE_SELL_STOP)
      {
         double orig_tp_dist = orig_entry - orig_tp;   // positive for SELL
         retry_tp = NormPrice(orig_entry - orig_tp_dist * hcfg.tp_mult);
      }
      else
      {
         retry_tp = orig_tp;  // fallback
      }

      // Determine the pending-stop type to retry with (must match original).
      // Only retry STOP-type pendings (BUY_STOP / SELL_STOP); skip market/limit/etc.
      ENUM_ORDER_TYPE retry_type;
      if(orig_type == ORDER_TYPE_BUY_STOP)        retry_type = ORDER_TYPE_BUY_STOP;
      else if(orig_type == ORDER_TYPE_SELL_STOP)  retry_type = ORDER_TYPE_SELL_STOP;
      else
      {
         PrintFormat("[%s] hedge SKIPPED: parent order type %d not stop (no retry)",
      ...
      int stopsLvl = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
      double pt = _Point;
      if(retry_type == ORDER_TYPE_BUY_STOP)
      {
         double minBuy = NormPrice(ask + stopsLvl * pt);
         if(orig_entry < minBuy)
         {
            PrintFormat("[%s] hedge SKIPPED: BUY_STOP entry %.5f below minBuy %.5f (stops level)",
                        hcfg.comment, orig_entry, minBuy);
            continue;
         }
         if(orig_entry <= ask)
         {
            PrintFormat("[%s] hedge SKIPPED: BUY_STOP entry %.5f <= ask %.5f (price above entry already)",
      ...
      {
         double maxSell = NormPrice(bid - stopsLvl * pt);
         if(orig_entry > maxSell)
         {
            PrintFormat("[%s] hedge SKIPPED: SELL_STOP entry %.5f above maxSell %.5f (stops level)",
                        hcfg.comment, orig_entry, maxSell);
            continue;
         }
         if(orig_entry >= bid)
         {
            PrintFormat("[%s] hedge SKIPPED: SELL_STOP entry %.5f >= bid %.5f (price below entry already)",
      ...
      bool ok = false;
      if(retry_type == ORDER_TYPE_BUY_STOP)
         ok = g_trade.BuyStop(lots, orig_entry, _Symbol, orig_sl, retry_tp,
                              ORDER_TIME_SPECIFIED, expire_broker, hcfg.comment);
      else
         ok = g_trade.SellStop(lots, orig_entry, _Symbol, orig_sl, retry_tp,
                               ORDER_TIME_SPECIFIED, expire_broker, hcfg.comment);
      if(!ok)
         PrintFormat("[%s] hedge RETRY failed (parent magic=%d pos_id=%I64d ticket=%I64u): %s",
                     hcfg.comment, hcfg.parent_magic, pos_id, dt,
                     g_trade.ResultRetcodeDescription());
      else
         PrintFormat("[%s] hedge RETRY %s placed at %.5f sl=%.5f tp=%.5f lots=%.2f "
                     "(parent pos_id=%I64d, orig order=%I64u)",
                     hcfg.comment,
                     (retry_type == ORDER_TYPE_BUY_STOP ? "BUY_STOP" : "SELL_STOP"),
                     orig_entry, orig_sl, retry_tp, lots, pos_id, orig_order);
   }
...
int OnInit()
{
   ...
   #define BUILD_HEDGE_CFG(N) \
      ...
      g_hedge_s##N.buffer_pts             = _HEDGE_S##N##_BufferPts; \
   ...
   PrintFormat("[DT818_pro_v2.1_h] Init. Parents S1=%d S2=%d S3=%d S4=%d S5=%d S6=%d  "
               "Hedges H1=%d H2=%d H3=%d H4=%d H5=%d H6=%d  "
               "Sessions LDN_hr=%d NY_hr=%d (real UTC, v2.1 broker offset patches active, "
               "broker=%+.1fh vs UTC; hedge order_type=STOP)",
...
void OnTick()
{
   ...
   // fires opposite-direction STOP orders for continuation.
