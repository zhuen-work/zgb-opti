//+------------------------------------------------------------------+
//| DT818_pro_v3.mq5                                                  |
//| ORB EA: 6-stream rank-portfolio + 6 REVERSE hedge sub-streams     |
//|                                                                   |
//| v3 changes vs v2.1_h (2026-05-14):                                |
//|   v2.1_h used RETRY hedges (same-direction STOP at parent entry). |
//|   v3 uses    REVERSE hedges (opposite-direction LIMIT at parent   |
//|              entry, mirrored SL/TP).                              |
//|                                                                   |
...
//|                                                                   |
//| Hedge magic numbers: 8111/8222/8333/8444/8555/8666 (v3 reverse).  |
//|   Parents stay 1111-6666. v2.1_h retry-hedge magics 7111-7666     |
//|   reserved for legacy. v3 reverse-hedge uses 8xxx so live logs    |
//|   can distinguish retry-vs-reverse sub-streams retroactively.     |
//|                                                                   |
...
void ProcessHedgeStream(HedgeStreamCfg &hcfg)
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      // ===== REVERSE-HEDGE GEOMETRY =====
      // Mirror parent's SL/TP across the entry, then flip direction.
      //
      // Parent BUY:  entry above SL,  TP above entry  → reverse SELL: SL above entry,  TP below entry
      ...
      if(hedge_type == ORDER_TYPE_BUY_LIMIT)
         ok = g_trade.BuyLimit(lots, orig_entry, _Symbol, hedge_sl, hedge_tp,
                                ORDER_TIME_SPECIFIED, expire_broker, hcfg.comment);
      else  // SELL_LIMIT
         ok = g_trade.SellLimit(lots, orig_entry, _Symbol, hedge_sl, hedge_tp,
                                 ORDER_TIME_SPECIFIED, expire_broker, hcfg.comment);
      if(!ok)
         PrintFormat("[%s] reverse-hedge FAILED (parent magic=%d pos_id=%I64d ticket=%I64u): %s",
...
int OnInit()
{
   ...
   PrintFormat("[DT818_pro_v3] Init. Parents S1=%d S2=%d S3=%d S4=%d S5=%d S6=%d  "
