...
//|                                                                   |
//| v2.2 PATCH (2026-05-13): Configurable range filter mode.          |
//|                                                                   |
//| NEW INPUT: _ORB_RangeFilterMode (default 0 = NONE)                |
//|   0 = NONE       — no range filter (place pendings regardless)    |
//|   1 = MIN_ONLY   — skip if range < _ORB_MinRangePts                |
//|                    (no max-side filter)                            |
//|   2 = MAX_ONLY   — skip if range > _ORB_MaxRangePts                |
//|                    (no min-side filter)                            |
//|                                                                   |
//| v2.1 enforced BOTH min AND max filters together. v2.2 lets you    |
//| enable just one (or neither) for WFO comparison. The "range"      |
//| being filtered is range_high - range_low (== distance between     |
//| BUY_STOP and SELL_STOP pending entry prices when buffer_pts=0).   |
//|                                                                   |
...
input int              _ORB_RangeFilterMode     = 0;    // v2.2: 0=NONE, 1=MIN_ONLY, 2=MAX_ONLY (only one filter active)
...
void UpdateORBSession(ORBSession &s, const ORBStreamCfg &cfg, datetime now)
{
   ...
   if(!s.fired && s.range_high > 0 && s.range_low > 0)
   {
      ...
      // v2.2: only one filter active at a time, selected by _ORB_RangeFilterMode
      //   0=NONE      no filter
      //   1=MIN_ONLY  skip if range too narrow (BUY/SELL stops too close)
      //   2=MAX_ONLY  skip if range too wide  (BUY/SELL stops too far apart)
      if(_ORB_RangeFilterMode == 1 && range_pts < _ORB_MinRangePts)
      { s.fired = true; return; }
      if(_ORB_RangeFilterMode == 2 && range_pts > _ORB_MaxRangePts)
      { s.fired = true; return; }
...
int OnInit()
{
   ...
   string filter_mode_str = "NONE";
   int filter_threshold = 0;
   if(_ORB_RangeFilterMode == 1) { filter_mode_str = "MIN_ONLY"; filter_threshold = _ORB_MinRangePts; }
   else if(_ORB_RangeFilterMode == 2) { filter_mode_str = "MAX_ONLY"; filter_threshold = _ORB_MaxRangePts; }
   PrintFormat("[DT818_pro_v2.2] Init. Streams S1=%d S2=%d S3=%d S4=%d S5=%d S6=%d  "
               "Sessions LDN_hr=%d NY_hr=%d  RangeFilter=%s at %dpt  "
               "(v2.1 broker offset patches inherited; broker=%+.1fh vs UTC)",
               ...
               filter_mode_str, filter_threshold,
               broker_off / 3600.0);
