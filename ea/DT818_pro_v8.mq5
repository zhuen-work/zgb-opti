...
//|         + SMA(8)x(21) cross-exit on post-HTP runners              |
//|         + GLOBAL (portfolio-level) daily target/loss (NEW v8)     |
//|                                                                   |
//| v8 changes vs v7 (2026-05-28):                                    |
//|   NEW: GLOBAL daily target/loss caps — portfolio-level, summing   |
//|        REALIZED-only (booked, closed-deal) P&L across ALL EA      |
//|        magics (6 parents 1111-6666 + 6 hedges 8111-8666) since    |
//|        broker day-start. Open positions ride to their own SL/TP;  |
//|        the cap triggers on booked P&L, not floating MTM (user      |
//|        pref 2026-05-28 — avoids whipsaw-closing recovering trades).|
//|     - _GlobalDailyTargetPct: lock ENTIRE portfolio for the day    |
//|       once aggregate P&L >= bal_start * pct/100 (0 = disabled).    |
//|     - _GlobalDailyLossPct:   lock ENTIRE portfolio once aggregate |
//|       P&L <= -bal_start * pct/100 (0 = disabled).                 |
//|     - On fire: close ALL EA positions, cancel ALL EA pendings,    |
//|       and OnTick early-returns (no streams, no hedges) until the  |
//|       next broker-day rollover resets the lock.                   |
//|     - Distinct from the v7 PER-STREAM _ORB_S{N}_Daily{Target,Loss}|
//|       Pct caps, which still exist and work independently. Global  |
//|       is evaluated FIRST; if it locks, per-stream is moot.        |
//|     - Bal_start anchor = ACCOUNT_BALANCE at first tick of each    |
//|       broker day (same convention as per-stream caps).            |
//|                                                                   |
//| v7 changes vs v6 (2026-05-26):                                    |
//|   NEW: SMA(8) x SMA(21) cross-exit on M5 closes for post-HTP      |
//|        runners (LOCKED periods per V5 sweep + v7 WFO).            |
//|     - On each closed M5 bar, compute SMA(_ORB_SMA_FastPeriod)     |
//|       and SMA(_ORB_SMA_SlowPeriod) of the last N closes.          |
//|     - Detect cross direction vs previous bar's relationship.      |
//|     - For each parent magic with HalfTP_Ratio > 0:                |
//|         * If exactly ONE position with this magic is active       |
//|           (i.e. half-lot has closed via TP; runner remains), AND  |
//|         * Cross direction opposes runner direction, AND           |
//|         * Runner is in profit (current PnL > 0)                   |
//|       → close runner at market.                                   |
//|     - Hedges (8xxx) untouched.                                    |
//|   Rationale (output/wfo_orb_v7_smacross_may23/ + V5 sweep):       |
//|     V5 4-window OOS sweep showed s8_21 = only variant producing   |
//|     positive 4-wk NP gain (+$759) with 0 regressions <= -15%.     |
//|     v7 WFO confirms top-7 all 4/4 profitable with cross-exit on.  |
//|   Setfile inputs added (LOCKED, do not sweep):                    |
//|     _ORB_SMA_CrossExit    = true                                  |
//|     _ORB_SMA_FastPeriod   = 8                                     |
//|     _ORB_SMA_SlowPeriod   = 21                                    |
//|                                                                   |
...
// v8: GLOBAL (portfolio-level) daily caps — aggregate P&L across ALL EA magics
// (6 parents + 6 hedges) since broker day-start. When hit, the ENTIRE portfolio
// locks for the day: all EA positions closed, all EA pendings cancelled, OnTick
// early-returns until the next day. 0 = disabled. Independent of (and evaluated
// BEFORE) the per-stream _ORB_S{N}_Daily{Target,Loss}Pct caps.
input double  _GlobalDailyTargetPct    = 0.0;    // LOCKED OFF: profit target truncates trend edge (degrades NDD)
input double  _GlobalDailyLossPct      = 0.0;    // EA default OFF; setfiles enable it (9pct=4.5, 6pct=3.0). Circuit-breaker.
...
// v7: SMA(fast) x SMA(slow) cross-exit on M5 closes for post-HTP runners.
// When _ORB_SMA_CrossExit=true:
//   - On every closed M5 bar, detect SMA(fast) crossing SMA(slow).
//   - For each parent magic with HalfTP_Ratio > 0, if exactly ONE position
//     is open with that magic (half-lot closed via TP, runner remains), AND
//     cross direction opposes runner direction, AND runner is in profit,
//     close runner at market.
//   - Hedges (8xxx) are unaffected.
// Periods locked to (8, 21) per V5 sweep + v7 WFO 2026-05-26.
input bool             _ORB_SMA_CrossExit       = false;
input int              _ORB_SMA_FastPeriod      = 8;
input int              _ORB_SMA_SlowPeriod      = 21;
...
datetime g_lastBar_M5 = 0;       // v7: M5 bar tracker for SMA cross-exit
...
// v8: GLOBAL (portfolio-level) daily-cap state + the set of EA magics whose P&L
// is aggregated. g_ea_magics is filled in OnInit from the parent + hedge magic
// inputs so it always matches the configured (possibly non-standard) magics.
datetime g_global_day = 0;   double g_global_bal_start = 0.0;   bool g_global_lock = false;
int      g_ea_magics[12];     int g_ea_magic_count = 0;
...
//====================================================================
// v8: GLOBAL (portfolio-level) daily cap helpers.
//====================================================================
bool IsEAMagic(int magic)
{
   for(int i = 0; i < g_ea_magic_count; i++)
      if(g_ea_magics[i] == magic) return true;
   return false;
}

double SumRealizedTodayAllEA(datetime dayStart)
{
   double total = 0.0;
   if(!HistorySelect(dayStart, TimeCurrent())) return 0.0;
   for(int i = HistoryDealsTotal() - 1; i >= 0; i--)
   {
      ...
      if(tk == 0) continue;
      if(HistoryDealGetString(tk, DEAL_SYMBOL) != _Symbol) continue;
      if(!IsEAMagic((int)HistoryDealGetInteger(tk, DEAL_MAGIC))) continue;
      if(HistoryDealGetInteger(tk, DEAL_ENTRY) != DEAL_ENTRY_OUT) continue;
      total += HistoryDealGetDouble(tk, DEAL_PROFIT);
      total += HistoryDealGetDouble(tk, DEAL_SWAP);
      total += HistoryDealGetDouble(tk, DEAL_COMMISSION);
...
void CloseAllEAPositions()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(t == 0 || !PositionSelectByTicket(t)) continue;
      if(PositionGetString(POSITION_SYMBOL) != _Symbol) continue;
      if(!IsEAMagic((int)PositionGetInteger(POSITION_MAGIC))) continue;
      g_trade.PositionClose(t);
   }
}
...
void CancelAllEAPendings()
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(t == 0 || !OrderSelect(t)) continue;
      if(OrderGetString(ORDER_SYMBOL) != _Symbol) continue;
      if(!IsEAMagic((int)OrderGetInteger(ORDER_MAGIC))) continue;
      g_trade.OrderDelete(t);
   }
}

//====================================================================
// Global portfolio daily cap check. Returns true when the WHOLE portfolio
// is locked for the day. Aggregates REALIZED-only (booked, closed-deal) P&L
// today across all EA magics (parents + hedges) — open positions ride to
// their own SL/TP; the cap triggers on booked P&L, NOT floating MTM. On fire:
// close any still-open EA positions, cancel EA pendings, lock the day. Caller
// (OnTick) early-returns so no streams/hedges process.
//
// Realized-only chosen 2026-05-28 (user pref): avoids whipsaw-closing a
// floating-loss position that would have recovered to TP. Trade-off: a single
// intraday excursion is NOT bounded by the cap % (only booked losses are) —
// the cap acts once enough trades have CLOSED to cross the threshold.
//====================================================================
bool GlobalDailyCapCheck()
{
   datetime today = DayStart(TimeCurrent());
   if(today != g_global_day)
   {
      g_global_day      = today;
      g_global_bal_start= AccountInfoDouble(ACCOUNT_BALANCE);
      g_global_lock     = false;
   }
   if(g_global_lock) return true;
   if(_GlobalDailyTargetPct <= 0 && _GlobalDailyLossPct <= 0) return false;
   if(g_global_bal_start <= 0) return false;

   double pnl = SumRealizedTodayAllEA(today);   // REALIZED-only (booked); no unrealized MTM

   if(_GlobalDailyTargetPct > 0 && pnl >= g_global_bal_start * _GlobalDailyTargetPct / 100.0)
   {
      CloseAllEAPositions();
      CancelAllEAPendings();
      g_global_lock = true;
      PrintFormat("[DT818_pro_v8] GLOBAL daily TARGET hit at $%.2f (+%.2f%%) — "
                  "portfolio locked for the day.", pnl, pnl / g_global_bal_start * 100.0);
      return true;
   }
   if(_GlobalDailyLossPct > 0 && pnl <= -g_global_bal_start * _GlobalDailyLossPct / 100.0)
   {
      CloseAllEAPositions();
      CancelAllEAPendings();
      g_global_lock = true;
      PrintFormat("[DT818_pro_v8] GLOBAL daily LOSS hit at $%.2f (%.2f%%) — "
                  "portfolio locked for the day.", pnl, pnl / g_global_bal_start * 100.0);
      return true;
   }
   return false;
...
int OnInit()
{
   ...
   // v8: build the EA magic set (6 parents + 6 hedges) for global P&L aggregation,
   // and seed the global day-start balance anchor.
   g_ea_magics[0] = _ORB_S1_Magic;   g_ea_magics[1] = _ORB_S2_Magic;
   g_ea_magics[2] = _ORB_S3_Magic;   g_ea_magics[3] = _ORB_S4_Magic;
   g_ea_magics[4] = _ORB_S5_Magic;   g_ea_magics[5] = _ORB_S6_Magic;
   g_ea_magics[6] = _HEDGE_S1_Magic; g_ea_magics[7] = _HEDGE_S2_Magic;
   g_ea_magics[8] = _HEDGE_S3_Magic; g_ea_magics[9] = _HEDGE_S4_Magic;
   g_ea_magics[10]= _HEDGE_S5_Magic; g_ea_magics[11]= _HEDGE_S6_Magic;
   g_ea_magic_count = 12;
   g_global_bal_start = bal;

   long broker_off = (long)TimeCurrent() - (long)TimeGMT();
   PrintFormat("[DT818_pro_v8] Init. Parents S1=%d S2=%d S3=%d S4=%d S5=%d S6=%d  "
               ...
               (_ORB_SMA_CrossExit ? "true" : "false"),
               _ORB_SMA_FastPeriod, _ORB_SMA_SlowPeriod,
               broker_off / 3600.0);
   // Per-stream parent params — confirms which setfile is actually loaded
   PrintFormat("[DT818_pro_v8] Parents: "
   ...
   PrintFormat("[DT818_pro_v8] STOP-ext Hedges: "
   ...
   // v8: GLOBAL daily-cap config (portfolio-level, summing all EA magics)
   PrintFormat("[DT818_pro_v8] GLOBAL daily caps: TargetPct=%.2f%% LossPct=%.2f%% "
               "(0=disabled)  bal_start_anchor=$%.2f  ea_magics=%d  "
               "[per-stream _ORB_S{N}_Daily{Target,Loss}Pct still active independently]",
               _GlobalDailyTargetPct, _GlobalDailyLossPct, g_global_bal_start, g_ea_magic_count);
...
//====================================================================
// v7: SMA(fast) x SMA(slow) cross-exit on M5 closes
//
// Fires once per newly-closed M5 bar. For each parent magic with HalfTP_Ratio > 0,
// if exactly ONE open position exists with that magic (half-lot has closed via TP,
// runner remains) AND the cross direction opposes runner direction AND the runner
// is in profit, close at market.
//
// Cross detection: compare sign of (SMA[fast] - SMA[slow]) between the just-closed
// bar (shift=1) and the prior bar (shift=2). A flip is a cross.
//   prev_diff >= 0 AND cur_diff <  0 → BEARISH cross (close profitable LONGS)
//   prev_diff <  0 AND cur_diff >= 0 → BULLISH cross (close profitable SHORTS)
//====================================================================
void CheckSMACrossExitForMagic(int magic, double half_tp_ratio,
                                const string label, int cross_dir)
{
   // Skip streams without HTP -- cross-exit only operates on POST-HTP runners.
   if(half_tp_ratio <= 0.0) return;

   // Count active positions with this magic and capture the (presumed) runner.
   int active = 0;
   ulong runner_ticket = 0;
   int  runner_dir = 0;        // +1 BUY, -1 SELL
   double runner_profit = 0.0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong tk = PositionGetTicket(i);
      if(tk == 0) continue;
      ...
      active++;
      runner_ticket = tk;
      runner_dir = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) ? +1 : -1;
      runner_profit = PositionGetDouble(POSITION_PROFIT)
                    + PositionGetDouble(POSITION_SWAP);
   }
   if(active != 1) return;  // either no runner alive or both halves still open

   // Side filter: bearish (-1) closes LONGS (+1); bullish (+1) closes SHORTS (-1).
   if(cross_dir == -1 && runner_dir != +1) return;
   if(cross_dir == +1 && runner_dir != -1) return;

   // Profit filter (runner must be in profit, matching sim).
   if(runner_profit <= 0.0) return;

   // Close at market.
   if(!PositionSelectByTicket(runner_ticket)) return;
   g_trade.SetExpertMagicNumber(magic);
   bool ok = g_trade.PositionClose(runner_ticket);
   PrintFormat("[%s] v7 SMA-cross-exit fired (dir=%s runner=%s profit=$%.2f) close=%s",
               label,
               (cross_dir == -1 ? "BEARISH" : "BULLISH"),
               (runner_dir == +1 ? "BUY" : "SELL"),
               runner_profit,
               (ok ? "OK" : "FAIL"));
}

void CheckSMACrossExitOnNewM5Bar()
{
   if(!_ORB_SMA_CrossExit) return;

   // Detect new M5 bar (use shift=1 = just-closed bar).
   datetime cur_m5 = iTime(_Symbol, PERIOD_M5, 1);
   if(cur_m5 == 0 || cur_m5 == g_lastBar_M5) return;
   g_lastBar_M5 = cur_m5;

   int fast = _ORB_SMA_FastPeriod;
   int slow = _ORB_SMA_SlowPeriod;
   if(fast < 2 || slow <= fast) return;

   // Need: SMA at the just-closed bar (shift=1) AND prior bar (shift=2).
   // CopyBuffer is most idiomatic, but for simplicity we compute SMA directly
   // from closes using iClose() to avoid managing handles.
   int needed = slow + 1;  // closes at shifts 1..slow+1 give SMA_slow at shift 1 and 2
   double closes[];
   ArrayResize(closes, needed);
   for(int k = 0; k < needed; k++)
      closes[k] = iClose(_Symbol, PERIOD_M5, k + 1);  // shift=1 is most recent closed

   // SMA of `n` values starting at shift `start` (closes[start..start+n-1]).
   double sma_fast_cur = 0.0, sma_slow_cur = 0.0;
   double sma_fast_prev = 0.0, sma_slow_prev = 0.0;
   for(int k = 0; k < fast; k++) sma_fast_cur += closes[k];
   for(int k = 0; k < slow; k++) sma_slow_cur += closes[k];
   for(int k = 1; k < fast + 1; k++) sma_fast_prev += closes[k];
   for(int k = 1; k < slow + 1; k++) sma_slow_prev += closes[k];
   sma_fast_cur  /= (double)fast;
   sma_slow_cur  /= (double)slow;
   sma_fast_prev /= (double)fast;
   sma_slow_prev /= (double)slow;

   double prev_diff = sma_fast_prev - sma_slow_prev;
   double cur_diff  = sma_fast_cur  - sma_slow_cur;
   int cross_dir = 0;
   if(prev_diff >= 0.0 && cur_diff <  0.0) cross_dir = -1;  // bearish
   else if(prev_diff <  0.0 && cur_diff >= 0.0) cross_dir = +1;  // bullish
   if(cross_dir == 0) return;  // no cross this bar

   // Try cross-exit on each of the 6 parent streams.
   CheckSMACrossExitForMagic(g_cfg_s1.magic, g_cfg_s1.half_tp_ratio, "ORB_S1", cross_dir);
   CheckSMACrossExitForMagic(g_cfg_s2.magic, g_cfg_s2.half_tp_ratio, "ORB_S2", cross_dir);
   CheckSMACrossExitForMagic(g_cfg_s3.magic, g_cfg_s3.half_tp_ratio, "ORB_S3", cross_dir);
   CheckSMACrossExitForMagic(g_cfg_s4.magic, g_cfg_s4.half_tp_ratio, "ORB_S4", cross_dir);
   CheckSMACrossExitForMagic(g_cfg_s5.magic, g_cfg_s5.half_tp_ratio, "ORB_S5", cross_dir);
   CheckSMACrossExitForMagic(g_cfg_s6.magic, g_cfg_s6.half_tp_ratio, "ORB_S6", cross_dir);
}
...
void OnTick()
{
   ...
   // v8: GLOBAL (portfolio-level) daily cap — evaluated FIRST. When locked,
   // the whole portfolio is flat + halted for the day (no streams, no hedges).
   if(GlobalDailyCapCheck())
      return;
   ...
   // v7: SMA(fast) x SMA(slow) cross-exit on M5 closes — fires at most once
   // per closed M5 bar for each magic with HalfTP_Ratio > 0 (post-HTP runners).
   CheckSMACrossExitOnNewM5Bar();
