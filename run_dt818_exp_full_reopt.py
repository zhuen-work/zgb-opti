"""DT818_exp full reopt — forward (S1→S2→S3) + reverse (S3→S2→S1) passes.

Each phase uses complete sweep (Optimization=1, exhaustive).
Final stage of each pass has all 3 streams on — pool both passes,
deduplicate on full param set, print top 5 by NP (DD < 20%).
...
OUTPUT_DIR    = Path(f"output/dt818_exp_full_{IS_TAG}")
...
S3_PARAMS = [
    ("_take_profit3", 9000, 500, 2000, 25000),
    ("_stop_loss3",   7000, 500, 2000, 15000),
...
S3_DEFAULTS = {"_take_profit3": 9000, "_stop_loss3": 7000, "_EMA_Period3": 3, "_Bars3": 6}
...
ALL_PARAM_COLS = [
    ...
    "param__take_profit3", "param__stop_loss3", "param__EMA_Period3", "param__Bars3",
]
...
def _extract_s1(row):
    return {
        ...
        "_Bars":         _vi(row, "param__Bars",         S1_DEFAULTS["_Bars"]),
    }
...
def _extract_s2(row):
    return {
        ...
        "_Bars2":        _vi(row, "param__Bars2",        S2_DEFAULTS["_Bars2"]),
    }
...
def _extract_s3(row):
    return {
        ...
        "_Bars3":        _vi(row, "param__Bars3",        S3_DEFAULTS["_Bars3"]),
    }
...
def _build_val_ini(p, report_id):
    lines = "\n".join([
        ...
        "_Trade3=1", f"_time_frame3={TF3_ENUM}",
...
def _run_val(job_id, p, parse_forward_report):
    from zgb_opti.collector import copy_report_artifact, find_report_artifact
    from zgb_opti.launcher import run_mt5_job
    # Cache: skip if report already collected
    for ext in (".htm", ".html"):
...
def main():
    ...
    print("=" * 68)
    print(f"  DT818_exp full reopt  ({IS_TAG.upper()})")
    ...
    print(f"  Final: top {TOP_N} unique candidates (all streams on) from both passes")
    print("=" * 68)
    ...
    # Phase F3: S1+S2 fixed, sweep S3 — all streams on, collect full pool
    _, df_f3 = _run_phase(
    ...
    # Tag forward candidates with fixed S1+S2 params + locked S3 EMA/Bars
    df_f3 = df_f3.copy()
    ...
    print(f"  → S3 fixed: TP={s3['_take_profit3']}  SL={s3['_stop_loss3']}  EMA={s3['_EMA_Period3']}  Bars={s3['_Bars3']}")

    # Phase R2: S3 fixed, sweep S2
    ...
    print(f"  → S2 fixed: TP={s2r['_take_profit2']}  SL={s2r['_stop_loss2']}  EMA={s2r['_EMA_Period2']}  Bars={s2r['_Bars2']}")

    # Phase R3: S3+S2 fixed, sweep S1 — all streams on, collect full pool
    _, df_r3 = _run_phase(
    ...
    # Tag reverse candidates with fixed S2+S3 params + locked S1 Bars
    df_r3 = df_r3.copy()
    ...
    print(f"\n  {'#':<3}  {'NP':>10}  {'PF':>6}  {'DD':>6}  {'Tr':>4}  "
          f"{'S1 TP/SL/EMA/Bars':<22}  {'S2 TP/SL/EMA/Bars':<22}  {'S3 TP/SL/EMA/Bars'}")
    print("  " + "─" * 110)
    for rank, (_, row) in enumerate(top5.iterrows(), 1):
        ...
        tp3   = _vi(row, "param__take_profit3", 0)
        sl3   = _vi(row, "param__stop_loss3",   0)
        e3    = _vi(row, "param__EMA_Period3",  0)
        b3    = _vi(row, "param__Bars3",        0)
        s1_str = f"{tp1}/{sl1}/EMA{e1}/B{b1}"
        s2_str = f"{tp2}/{sl2}/EMA{e2}/B{b2}"
        s3_str = f"{tp3}/{sl3}/EMA{e3}/B{b3}"
        print(f"  {rank:<3}  {np_v:>+10,.0f}  {pf_v:>6.3f}  {dd_v:>5.1f}%  {tr_v:>4}  "
              f"{s1_str:<22}  {s2_str:<22}  {s3_str}")

    print(f"\n  Baseline (DT818_max): NP=+$41,397  PF=2.090  DD=11.8%  Tr=88")
    print("=" * 68)

    # ════════════════════════════════════════════════════════════════════════
    # ET VALIDATION — top 5 on Every Tick, $10k deposit
    # ════════════════════════════════════════════════════════════════════════
    print("\n" + "═" * 68)
    ...
    for rank, (_, row) in enumerate(top5.iterrows(), 1):
        p = dict(
            ...
            ema1=_vi(row, "param__EMA_Period1", 0), bars1=_vi(row, "param__Bars",        0),
            tp2=_vi(row, "param__take_profit2", 0), sl2=_vi(row, "param__stop_loss2",   0),
            ema2=_vi(row, "param__EMA_Period2", 0), bars2=_vi(row, "param__Bars2",       0),
            tp3=_vi(row, "param__take_profit3", 0), sl3=_vi(row, "param__stop_loss3",   0),
            ema3=_vi(row, "param__EMA_Period3", 0), bars3=_vi(row, "param__Bars3",       0),
        )
        print(f"\n── ET #{rank} ──")
    ...
    for r in et_results:
        ...
        s3_str = f"{p['tp3']}/{p['sl3']}/EMA{p['ema3']}/B{p['bars3']}"
        print(f"  {r['rank']:<4}  {r['np']:>+10,.0f}  {r['pf']:>6.3f}  {r['dd']:>5.1f}%  {r['trades']:>4}  {r['np_dd']:>8,.0f}  "
              f"{s1_str:<22}  {s2_str:<22}  {s3_str}")
    print("=" * 68)
