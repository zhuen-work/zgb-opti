"""Scalper v1 S1 Walk-Forward Optimization — Genetic (wider sweep).

IS:  Model=0, genetic Optimization=2, criterion=Recovery Factor
OOS: Model=4 (strict real ticks)
Risk fixed 1%. HedgeMode off. S2 disabled (removed from EA).

Windows (same as previous):
  W1: IS 2026-02-07 -> 2026-03-07   OOS 2026-03-07 -> 2026-03-21
  W2: IS 2026-02-21 -> 2026-03-21   OOS 2026-03-21 -> 2026-04-04
  W3: IS 2026-03-07 -> 2026-04-04   OOS 2026-04-04 -> 2026-04-18

Flow:
  A) IS genetic per window -> top-50 passes each
  B) Cross-window robust: params in top-50 of 2+ windows, dedupe by total NP -> top 5
  C) OOS validation on Model=4 (strict)
  D) Rank + write winner setfile
  E) Sanity ET full-span on Model=4
"""
...
OUTPUT_DIR    = Path("output/scalp_wfo_s1_genetic_apr18")
SET_OUT       = Path("configs/sets/scalp_v1_s1_genetic_apr18.set")
...
def _common_s1():
    """Common fixed S1 inputs (risk, hours, loss, hedge)."""
    return [
        _fix("_Magic", 2000),
        _fix("_EntryTF", 5),
        "_BlockFriPM=true",
...
def _base_params(donch, tp, sl, htp, tgt, peb):
    return _common_s1() + [
        _fix("_S1_DonchianBars", donch),
        _fix("_S1_TakeProfit", tp),
        _fix("_S1_StopLoss", sl),
        _fix("_S1_HalfTP_Ratio", htp),
        _fix("_S1_PendingExpireBars", peb),
        _fix("_S1_DailyTargetPct", tgt),
    ]


def _opt_params_sweep():
    """Genetic sweep — wider than previous exhaustive, genetic converges it fast."""
    return _common_s1() + [
        _sweep("_S1_DonchianBars",       25, 5,    20, 35),
        _sweep("_S1_TakeProfit",         200, 50,  100, 300),
        _sweep("_S1_StopLoss",           60, 10,   40, 100),
        _sweep("_S1_HalfTP_Ratio",       0.4, 0.2, 0.0, 0.8),
        _sweep("_S1_DailyTargetPct",     6, 3,     3, 9),
        _sweep("_S1_PendingExpireBars",  2, 1,     1, 3),
    ]
...
def run_window_optimization(win_name, is_start, is_end):
    """Run IS genetic, return top-50 passes as dict list."""
    from zgb_opti.xml_parser import parse_optimization_xml, write_passes
    import pandas as pd

    job_id = f"scalp_s1_gen_{win_name}_is"
    ...
    peb_c   = [c for c in df.columns if "S1_PendingExpireBars" in c][0]

    for c in [donch_c, tp_c, sl_c, htp_c, tgt_c, peb_c]:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    top50 = df.sort_values("rf", ascending=False).head(50)
    print(f"  Window {win_name}: top 5 by RF (of {len(df)} profitable passes):")
    print(f"    {'NP':>10}  {'PF':>6}  {'DD':>6}  {'Tr':>4}  {'RF':>6}  Donch  TP  SL  HTP  Tgt  PEB")
    for _, r in top50.head(5).iterrows():
        print(f"    {r['net_profit']:>+10,.0f}  {r['profit_factor']:>6.2f}  {r['drawdown_pct']:>5.1f}%  "
              ...
              f"{int(r[sl_c]):>3}  {r[htp_c]:>4.1f}  {int(r[tgt_c]):>3}  {int(r[peb_c]):>3}")
...
def run_et(params, oos_start, oos_end, job_id):
    ...
    if not htm.exists():
        set_lines = _base_params(params["donch"], params["tp"], params["sl"],
                                  params["htp"], params["tgt"], params["peb"])
...
def main():
    ...
    print("=" * 72)
    print("  Scalper v1 S1 Genetic WFO | IS=Model0 genetic | OOS=Model4 real ticks")
    print(f"  Windows: {len(WINDOWS)} × 4w IS / 2w OOS | Risk=1% | Deposit=${DEPOSIT}")
    print("=" * 72)

    # Phase A: IS genetic per window
    all_top = {}
    for win_name, is_s, is_e, _, _ in WINDOWS:
        all_top[win_name] = run_window_optimization(win_name, is_s, is_e)

    # Phase B: cross-window robust filter + dedupe by total NP
    def pk(p):
        return (p["donch"], p["tp"], p["sl"], p["htp"], p["tgt"], p["peb"])
    ...
    print(f"\n  Robust params (in top-50 of 2+ windows): {len(robust)}")
    ...
    # Phase C: OOS validation on Model=4
    print("\n" + "=" * 72)
    print("  PHASE C: OOS Validation on Model=4 (strict real ticks)")
    print("=" * 72)

    scores = []
    for i, (k, info) in enumerate(unique, 1):
        donch, tp, sl, htp, tgt, peb = k
        params = {"donch": donch, "tp": tp, "sl": sl, "htp": htp, "tgt": tgt, "peb": peb}
        print(f"\n  #{i} Donch={donch} TP={tp} SL={sl} HTP={htp} Tgt={tgt} PEB={peb}  (windows {info['windows']})")

        oos_total = {"np": 0, "tr": 0}
        oos_results = []
        all_prof = True
        for win_name, _, _, oos_s, oos_e in WINDOWS:
            job_id = f"scalp_s1_gen_oos_{win_name}_{i}"
    ...
    print("\n" + "=" * 72)
    print("  FINAL RANKING (by total OOS NP on real ticks)")
    print("=" * 72)
    ...
    for rank, s in enumerate(scores[:5], 1):
        ...
        print(f"  {rank:<4}  {s['oos_total_np']:>+13,.2f}  {flag:>8}  "
              f"Donch={p['donch']} TP={p['tp']} SL={p['sl']} HTP={p['htp']} Tgt={p['tgt']} PEB={p['peb']}")
    ...
    if scores:
        ...
        set_content = "\n".join([
            ...
            f"_S1_PendingExpireBars={p['peb']}||{p['peb']}||1||{p['peb']}||{p['peb']}||N",
        ...
        # Phase E: sanity ET full-span on Model=4
        print("\n" + "=" * 72)
        print("  PHASE E: Sanity ET full-span Mar 7 -> Apr 18 on Model=4")
        print("=" * 72)
        sanity_job = "scalp_s1_gen_sanity_full"
        sanity = run_et(p, date(2026,3,7), date(2026,4,18), sanity_job)
        if sanity:
