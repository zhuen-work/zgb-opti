"""DT818_inv reopt: S1=M5, S2=M15 (fade fractal entries on lower TF).
...
Chart period: M5 (finer of M5/M15).
"""
...
OUTPUT_DIR    = Path(f"output/dt818_inv_m5xm15_{IS_TAG}")
...
def _joint_set_lines():
    return [
        ...
        _sweep("_take_profit",  3000, 250,  500,  10000),
        _sweep("_stop_loss",    3000, 250,  500,  10000),
        _sweep("_EMA_Period1",  5,    1,    3,    10),
        _sweep("_Bars",         3,    1,    2,    6),
        _fix("_RiskMode1",    0),
        _fix("_TSL1",         500),
        _fix("_TSLA1",        200),
        ...
        _sweep("_take_profit2",  3000, 250,  500,  10000),
        _sweep("_stop_loss2",    3000, 250,  500,  10000),
        _sweep("_EMA_Period2",   5,    1,    3,    10),
        _sweep("_Bars2",         3,    1,    2,    6),
        _fix("_RiskMode2",    0),
        _fix("_TSL2",         4500),
        _fix("_TSLA2",        1000),
...
def _extract_params(row):
    ...
    return dict(
        take_profit  = vi("param__take_profit",  3000),
        stop_loss    = vi("param__stop_loss",    3000),
        EMA_Period1  = vi("param__EMA_Period1",  5),
        Bars         = vi("param__Bars",         3),
        take_profit2 = vi("param__take_profit2", 3000),
        stop_loss2   = vi("param__stop_loss2",   3000),
        EMA_Period2  = vi("param__EMA_Period2",  5),
        Bars2        = vi("param__Bars2",        3),
    )
...
def _run_attempt(attempt, job_id, pd, parse_optimization_xml, write_passes, parse_forward_report):
    ...
    if xml_path.exists():
        ...
        print(f"\n── Attempt {attempt}/{MAX_ATTEMPTS}: joint genetic (1m OHLC, $10M, M5×M15) ──")
    ...
    for rank, (_, row) in enumerate(top_n.iterrows(), 1):
        ...
        val_id = f"val_et_inv_m5xm15_{IS_TAG}_a{attempt}_{rank}"
...
def main():
    ...
    print("=" * 65)
    print(f"  DT818_inv M5×M15 reopt  ({IS_TAG.upper()})")
    print(f"  IS: {IS_START} -> {IS_END}")
    print(f"  S1=M5  S2=M15  Chart={PERIOD}  EA=DT818_inv (fade fractals)")
    ...
    job_id = f"opti_inv_m5xm15_{IS_TAG}"
    ...
    print(f"\n{'=' * 65}")
    print(f"  DONE  —  DT818_inv  M5×M15  ({IS_TAG.upper()})")
    print(f"  S1 (M5):  TP={p['take_profit']}  SL={p['stop_loss']}  EMA={p['EMA_Period1']}  Bars={p['Bars']}")
    print(f"  S2 (M15): TP={p['take_profit2']}  SL={p['stop_loss2']}  EMA={p['EMA_Period2']}  Bars2={p['Bars2']}")
